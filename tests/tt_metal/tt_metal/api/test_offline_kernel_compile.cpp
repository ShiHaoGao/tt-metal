...
TEST_F(OfflineKernelCompileMockFixture, CPU_RuntimeKernelAllowsWatcherOptionsWithoutMockWatcher) {
    auto& context = MetalContext::instance();
    ASSERT_EQ(context.watcher_server().get(), nullptr);
    auto& options = context.rtoptions();
    const bool was_enabled = options.get_watcher_enabled();
    options.set_watcher_enabled(true);
    const KernelSource source = KernelSource::from_source("void kernel_main() {}");
    const CoreRangeSet cores(CoreRange{CoreCoord{0, 0}, CoreCoord{0, 0}});
    EXPECT_NO_THROW({
        const DataMovementKernel kernel(DEFAULT_CONTEXT_ID, source, cores, kReaderDmConfig);
        EXPECT_EQ(kernel.get_watcher_kernel_id(), -1);
    });
    EXPECT_NO_THROW({
        const ComputeKernel kernel(DEFAULT_CONTEXT_ID, source, cores, ComputeConfig{});
        EXPECT_EQ(kernel.get_watcher_kernel_id(), -1);
    });
    options.set_watcher_enabled(was_enabled);
}
...
TEST_F(OfflineKernelCompileMockFixture, CPU_CompileKernelOfflineRejectsIncompleteExplicitEnvironment) {
    using Params = experimental::OfflineKernelCompileParams;
    Params params{
