...
constexpr uint32_t streaming_qktv_h(
    uint32_t subblock_h,
    uint32_t subblock_w,
    uint32_t dst_size,
    uint32_t Sq_chunk_t,
    uint32_t max_qktv_h = 0xffffffffu) {
    ...
    uint32_t qktv_h = (subblock_h == 1 && 2 * subblock_w <= dst_size && Sq_chunk_t >= 2) ? 2u : subblock_h;
    if (qktv_h > max_qktv_h) {
        qktv_h = max_qktv_h;
    }
    return qktv_h == 0 ? 1u : qktv_h;
}
