...
void kernel_main() {
    ...
    constexpr uint32_t qk_detail_profile_stage = get_compile_time_arg_val(32);
    constexpr uint32_t q_reader_schedule = get_compile_time_arg_val(33);
    ...
    if constexpr (is_chunked) {
        if (chunk_start_idx_addr != 0) {
            DeviceZoneScopedN("FAP_READER_CHUNK_START");
    ...
    for (uint32_t phase = 0; phase < num_phases; ++phase) {
        ...
        for (uint32_t nb = local_batch_start; nb < local_batch_end; ++nb) {
            if constexpr (is_chunked) {
                ...
                DeviceZoneScopedN("FAP_READER_PAGE_TABLE");
            ...
            for (uint32_t nq = local_nh_start; nq < local_nh_end; ++nq) {
                ...
                if constexpr (use_attention_sink) {
                    DeviceZoneScopedN("FAP_READER_ATTENTION_SINK");
                ...
                for (uint32_t q_iter = 0; q_iter < q_chunks_per_core; ++q_iter) {
                    ...
                    */
                    ...
                    if constexpr (!use_q_subblock_push) {
                        DeviceZoneScopedN("FAP_READER_Q");
                    ...
                    if constexpr (use_q_subblock_push && q_reader_schedule == 1) {
                        DeviceZoneScopedN("FAP_READER_Q_EARLY_FIRST");
                        read_q_subblock<q_tile_bytes, qk_detail_profile_stage == 2>(
                            q_reader,
                            cb_q_in,
                            q_read_tile_id,
                            0,
                    ...
                    for (uint32_t k_chunk = 0; (k_chunk * Sk_chunk_t) < q_high_idx; ++k_chunk) {
                        ...
                        if (should_receive) {
                            ...
                            DeviceZoneScopedN("FAP_READER_K_RECV");
                            ...
                            DeviceZoneScopedN("FAP_READER_K_READ");
                            if constexpr (is_chunked) {
                                ...
                                if constexpr (use_q_subblock_push && q_reader_schedule == 2) {
                                    if (k_chunk == 0 && q_subblocks_pushed == 0) {
                                        DeviceZoneScopedN("FAP_READER_K_READ_Q_DURING_FINAL_BARRIER");
                                        cb_reserve_back(cb_k_in, k_chunk_tiles);
                                        const uint32_t k_base_write_ptr = get_write_ptr(cb_k_in);
                                        static_assert(qk_subblock_h * DHt > 0);
                                        uint32_t barrier_count = 0;
                                        uint32_t issued_k_tiles = 0;
                                        const uint32_t total_k_tiles = Sk_chunk_t * DHt;
                                        auto maybe_barrier_before_final_batch = [&]() {
                                            issued_k_tiles++;
                                            if (barrier_count == barrier_threshold && issued_k_tiles < total_k_tiles) {
                                                noc_async_read_barrier();
                                                barrier_count = 0;
                                            }
                                        };

                                        for (uint32_t row = 0; row < kv_row_tile_count; ++row) {
                                            uint32_t write_ptr = k_base_write_ptr + row * k_tile_bytes;
                                            uint32_t physical_tile_id =
                                                virtual_seq_tile_id_to_physical_tile_id<uint32_t, NKH, block_size_t, DHt>(
                                                    k_chunk_start_row_num + row, k_head, page_table_ptr);
                                            for (uint32_t col = 0; col < DHt; ++col) {
                                                noc_async_read_tile(physical_tile_id++, k_reader, write_ptr);
                                                write_ptr += k_tile_bytes * Sk_chunk_t;
                                                barrier_count++;
                                                maybe_barrier_before_final_batch();
                                            }
                                        }
                                        for (uint32_t row = 0; row < Sk_chunk_t; ++row) {
                                            for (uint32_t col = 0; col < DHt; ++col) {
                                                ...
                                                const uint32_t tile_id = col * Sk_chunk_t + row;
                                                fill_zeros_async(k_base_write_ptr + tile_id * k_tile_bytes, k_tile_bytes);
                                                barrier_count++;
                                                maybe_barrier_before_final_batch();
                                            }
                                        }
                                        ...
                                        {
                                            DeviceZoneScopedN("FAP_READER_Q_DURING_K_READ");
                                            read_q_subblock<q_tile_bytes, qk_detail_profile_stage == 2>(
                                                q_reader,
                                                cb_q_in,
                                                q_read_tile_id,
                        ...
                        if (should_forward) {
                            DeviceZoneScopedN("FAP_READER_K_FORWARD");
                        ...
                        if constexpr (use_provided_mask) {
                            DeviceZoneScopedN("FAP_READER_MASK");
                        ...
                        if constexpr (use_q_subblock_push) {
                            if (k_chunk == 0) {
                                DeviceZoneScopedN("FAP_READER_Q");
                                for (uint32_t q_sub = q_subblocks_pushed; q_sub < q_num_subblocks; ++q_sub) {
                                    read_q_subblock<q_tile_bytes, qk_detail_profile_stage == 2>(
                                        q_reader,
                                        cb_q_in,
                                        q_read_tile_id,
                        ...
                        if (should_receive) {
                            ...
                            DeviceZoneScopedN("FAP_READER_V_RECV");
                            ...
                            DeviceZoneScopedN("FAP_READER_V_READ");
                            if constexpr (is_chunked) {
                        ...
                        if (should_forward) {
                            DeviceZoneScopedN("FAP_READER_V_FORWARD");
                            noc_semaphore_wait(sender_semaphore_addr_ptr, sender_wait_count);
                            noc_semaphore_set(sender_semaphore_addr_ptr, 0);
                            if constexpr (mcast_enabled) {
