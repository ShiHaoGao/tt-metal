...
#include "tt_metal/programming_examples/profiler/flash_attention_profile/kernels/sdpa_streaming_qktv.hpp"
...
#define FAP_CONCAT_INNER(a, b) a##b
#define FAP_CONCAT(a, b) FAP_CONCAT_INNER(a, b)
#define MaybeDeviceZoneScopedN(ENABLED, name)                                  \
    DO_PRAGMA(message(PROFILER_MSG_NAME(name)));                               \
    MaybeProfileScope<ENABLED, kernel_profiler::Hash16_CT(PROFILER_MSG_NAME(name))> FAP_CONCAT(maybe_zone_, __LINE__);
#else
#define MaybeDeviceZoneScopedN(ENABLED, name)
#endif
...
template <bool profile_pack_thread = false, uint32_t qk_detail_profile_stage = 0>
ALWI void pack_contiguous_rows_nocfg(
    ...
    constexpr bool profile_qk_pack_thread = profile_pack_thread && qk_detail_profile_stage == 1;
    constexpr bool profile_qktv_pack_thread = profile_pack_thread && qk_detail_profile_stage == 7;
    constexpr bool profile_qktv_pack_detail = profile_pack_thread && qk_detail_profile_stage == 9;
    MaybeDeviceZoneScopedN(profile_qk_pack_thread, "FAP_COMPUTE_QK_BODY_PACK_THREAD");
    MaybeDeviceZoneScopedN(profile_qktv_pack_thread, "FAP_COMPUTE_QKTV_BODY_PACK_THREAD");
    MaybeDeviceZoneScopedN(profile_qktv_pack_detail, "FAP_COMPUTE_QKTV_BODY_PACK_THREAD");
#endif
...
template <bool profile_matmul_threads = false>
ALWI void qk_matmul_block_no_mop(
    uint32_t in0_cb_id,
    uint32_t in1_cb_id,
    ...
    {
        MaybeDeviceZoneScopedN(profile_matmul_threads, "FAP_COMPUTE_QK_BODY_UNPACK_AB");
        llk_unpack_AB_matmul(in0_cb_id, in1_cb_id, in0_tile_index, in1_tile_index, ct_dim, rt_dim, kt_dim);
    }
#endif
#if defined(TRISC_MATH)
    {
        MaybeDeviceZoneScopedN(profile_matmul_threads, "FAP_COMPUTE_QK_BODY_MATH_MATMUL");
        llk_math_matmul_no_mop<MATH_FIDELITY, MM_THROTTLE>(idst, ct_dim, rt_dim);
    }
#endif
}

template <bool profile_matmul_threads = false, uint32_t qk_detail_profile_stage = 0>
ALWI void qktv_matmul_block_no_mop(
    uint32_t in0_cb_id,
    uint32_t in1_cb_id,
    uint32_t in0_tile_index,
    uint32_t in1_tile_index,
...
#if defined(TRISC_UNPACK)
    {
        MaybeDeviceZoneScopedN(profile_matmul_threads && qk_detail_profile_stage == 7, "FAP_COMPUTE_QKTV_BODY_UNPACK_AB");
        MaybeDeviceZoneScopedN(profile_matmul_threads && qk_detail_profile_stage == 8, "FAP_COMPUTE_QKTV_MATH_DETAIL_UNPACK_AB");
        llk_unpack_AB_matmul(in0_cb_id, in1_cb_id, in0_tile_index, in1_tile_index, ct_dim, rt_dim, kt_dim);
    }
#endif
#if defined(TRISC_MATH)
    {
        MaybeDeviceZoneScopedN(profile_matmul_threads && qk_detail_profile_stage == 7, "FAP_COMPUTE_QKTV_BODY_MATH_MATMUL");
        MaybeDeviceZoneScopedN(profile_matmul_threads && qk_detail_profile_stage == 8, "FAP_COMPUTE_QKTV_MATH_DETAIL_MATMUL");
        llk_math_matmul_no_mop<MATH_FIDELITY, MM_THROTTLE>(idst, ct_dim, rt_dim);
    }
#endif
}
...
SDPA_NOINLINE void blocked_matmul_and_pack(
    ...
    constexpr bool profile_qk_body_detail = profile_qk_body && qk_detail_profile_stage == 1;
    constexpr bool profile_qktv_threads = profile_qk_body && qk_detail_profile_stage == 7;
    constexpr bool profile_qktv_math_detail = profile_qk_body && qk_detail_profile_stage == 8;
    constexpr bool profile_qktv_pack_detail = profile_qk_body && qk_detail_profile_stage == 9;
    {
        MaybeDeviceZoneScopedN(profile_qk_body_detail, "FAP_COMPUTE_QK_BODY_TILE_REGS_ACQUIRE");
        MaybeDeviceZoneScopedN(profile_qktv_math_detail, "FAP_COMPUTE_QKTV_BODY_TILE_REGS_ACQUIRE");
        tile_regs_acquire();
    }
    uint32_t dst_index = 0;
    ...
    {
        MaybeDeviceZoneScopedN(profile_qk_body_detail, "FAP_COMPUTE_QK_BODY_MATMUL_LOOP");
        MaybeDeviceZoneScopedN(profile_qktv_math_detail, "FAP_COMPUTE_QKTV_BODY_MATMUL_LOOP");
        for (uint32_t inner = 0; inner < inner_dim; ++inner) {
            if constexpr (profile_qk_body_detail) {
                qk_matmul_block_no_mop<true>(
                    in0_cb, in1_cb, in0_index, in1_index, dst_index, transpose, subblock_w, subblock_h, matmul_stride);
            } else if constexpr (profile_qktv_threads || profile_qktv_math_detail) {
                qktv_matmul_block_no_mop<true, qk_detail_profile_stage>(
                    in0_cb, in1_cb, in0_index, in1_index, dst_index, transpose, subblock_w, subblock_h, matmul_stride);
            } else {
                matmul_block_no_mop(
                    in0_cb,
    ...
    {
        MaybeDeviceZoneScopedN(profile_qk_body_detail, "FAP_COMPUTE_QK_BODY_TILE_REGS_COMMIT");
        MaybeDeviceZoneScopedN(profile_qktv_math_detail, "FAP_COMPUTE_QKTV_BODY_TILE_REGS_COMMIT");
        tile_regs_commit();
    }

    {
        MaybeDeviceZoneScopedN(profile_qk_body_detail, "FAP_COMPUTE_QK_BODY_TILE_REGS_WAIT");
        MaybeDeviceZoneScopedN(profile_qktv_math_detail, "FAP_COMPUTE_QKTV_BODY_TILE_REGS_WAIT");
        tile_regs_wait();
    }
    if (!skip_pack_configure) {
        MaybeDeviceZoneScopedN(profile_qk_body_detail, "FAP_COMPUTE_QK_BODY_PACK_CONFIG");
        MaybeDeviceZoneScopedN(profile_qktv_pack_detail, "FAP_COMPUTE_QKTV_BODY_PACK_CONFIG");
        configure_row_pack_width(out_cb, subblock_w);
    }
    {
        MaybeDeviceZoneScopedN(profile_qk_body_detail, "FAP_COMPUTE_QK_BODY_PACK_ROWS");
        MaybeDeviceZoneScopedN(profile_qktv_pack_detail, "FAP_COMPUTE_QKTV_BODY_PACK_ROWS");
        pack_contiguous_rows_nocfg<
            profile_qk_body_detail || profile_qktv_threads || profile_qktv_pack_detail,
            qk_detail_profile_stage>(
            out_cb, row_subblock_idx * subblock_h, subblock_h, out_num_cols, out_col_offset, subblock_w);
    }
    if (trigger_reduce) {
        MaybeDeviceZoneScopedN(profile_qk_body_detail, "FAP_COMPUTE_QK_BODY_REDUCE_SIGNAL");
        PACK((t6_semaphore_post<p_stall::NONE>(semaphore::FPU_SFPU)));
    }
    {
        MaybeDeviceZoneScopedN(profile_qk_body_detail, "FAP_COMPUTE_QK_BODY_TILE_REGS_RELEASE");
        MaybeDeviceZoneScopedN(profile_qktv_math_detail, "FAP_COMPUTE_QKTV_BODY_TILE_REGS_RELEASE");
        tile_regs_release();
    }
}

template <uint32_t cb_identity_scale_in, uint32_t scratch_cb>
SDPA_NOINLINE void qk_first_body_tiny_matmul_warmup() {
    DeviceZoneScopedN("FAP_COMPUTE_QK_FIRST_BODY_WARMUP");
    cb_reserve_back(scratch_cb, 1);
    mm_no_mop_init_short(cb_identity_scale_in, cb_identity_scale_in, false, 1, 1, 1);
    blocked_matmul_and_pack<false, 1, 1, false, 0>(
...
template <
    ...
    uint32_t DHt,
    bool uniform_dataformat>
SDPA_NOINLINE void qk_first_body_same_config_init(uint32_t actual_sbw) {
    DeviceZoneScopedN("FAP_COMPUTE_QK_FIRST_BODY_SAME_CONFIG_INIT");
    ...
    mm_no_mop_init_short(cb_q_in, cb_kt_in, true, actual_sbw, qkt_subblock_h, DHt);
 ...
 */
template <uint32_t scale_fp32, uint32_t qk_softmax_profile_stage = 0>
SDPA_NOINLINE void sub_exp_block_bcast_cols(
    ...
    if constexpr (qk_softmax_profile_stage == 1) {
        MaybeDeviceZoneScopedN(true, "FAP_COMPUTE_QK_SOFTMAX_WAIT_MAX");
    ...
    tile_regs_acquire();
    if constexpr (qk_softmax_profile_stage == 2) {
        MaybeDeviceZoneScopedN(true, "FAP_COMPUTE_QK_SOFTMAX_SUB_MATH");
        uint32_t dst_index = 0;
    ...
    if constexpr (qk_softmax_profile_stage == 3) {
        MaybeDeviceZoneScopedN(true, "FAP_COMPUTE_QK_SOFTMAX_WAIT_SUB");
        tile_regs_wait();
    } else {
        tile_regs_wait();
    }
    PACK((llk_pack_relu_config(ReluType::ZERO_RELU)));
    if constexpr (qk_softmax_profile_stage == 4) {
        MaybeDeviceZoneScopedN(true, "FAP_COMPUTE_QK_SOFTMAX_EXP_SFPU");
        uint32_t dst_index = 0;
...
#pragma GCC unroll 1
    ...
    if constexpr (qk_softmax_profile_stage == 5) {
        MaybeDeviceZoneScopedN(true, "FAP_COMPUTE_QK_SOFTMAX_PACK");
        pack_outputs();
    } else {
        pack_outputs();
    }
...
template <
    ...
    uint32_t qktv_h_limit = 0xffffffffu,
    bool salad_first_handoff = false,
...
static void sdpa_inner_loop_step(
    ...
    constexpr bool qktv_detail_profile = qk_detail_profile_stage == 5;
    constexpr bool qktv_barrier_split_profile = qk_detail_profile_stage == 6;
    constexpr bool qktv_matmul_detail_profile = qk_detail_profile_stage == 7;
    constexpr bool qktv_math_detail_profile = qk_detail_profile_stage == 8;
    constexpr bool qktv_pack_detail_profile = qk_detail_profile_stage == 9;
    ...
    const bool qk_init_is_primed = (qk_first_body_warmup == 2) && is_first_iter;
    if constexpr (qk_first_body_warmup == 2) {
        if (is_first_iter) {
            qk_first_body_same_config_init<
                cb_q_in,
                cb_kt_in,
                cb_qkt_im,
                qkt_subblock_h,
                DHt,
                uniform_dataformat>(actual_sbw);
        }
    }

    constexpr bool phase_timeline_profile = qk_detail_profile_stage == 4;
    constexpr uint32_t qktv_h =
        ttnn::transformer::sdpa::streaming_qktv_h(
            qktv_subblock_h, qktv_subblock_w, dst_size, Sq_chunk_t, qktv_h_limit);
    ...
    constexpr uint32_t qktv_q_num_subblocks = Sq_chunk_t / qktv_h;
    ...
    static_assert(
        compute_pipeline_schedule == 0 || qktv_h == qkt_subblock_h,
        "compute pipeline experiments require qktv_h == qkt_subblock_h");
    static_assert(
        compute_pipeline_schedule == 0 || !salad_first_handoff,
        "compute pipeline experiments are only defined for the default SALAD handoff order");
    static_assert(
        compute_pipeline_schedule != 4 || qk_softmax_schedule == 0,
        "fa3_pv_softmax_v1 is only defined for the default QK softmax schedule");
    static_assert(
        compute_pipeline_schedule != 3 || qktv_q_num_subblocks > 1,
        "group0_early_push_v1 requires at least two QKTV row groups");
    static_assert(
    ...
    bool qktv_v_waited = false;
    uint32_t early_qktv_groups = 0;

    auto ensure_qktv_output_reserved = [&]() {
        if (!qktv_output_reserved) {
            cb_reserve_back(out_cb, qktv_output_num_tiles);
            qktv_output_reserved = true;
        }
    };

    auto ensure_qktv_v_ready = [&]() {
        if (!qktv_v_waited) {
            DeviceZoneScopedN("FAP_COMPUTE_QKTV_WAIT_V");
            cb_wait_front(cb_v_in, Sk_chunk_t * vDHt);
            qktv_v_waited = true;
        }
    };

    auto qktv_pack_unpack_barrier = [&]() {
        MaybeDeviceZoneScopedN(qktv_detail_profile, "FAP_COMPUTE_QKTV_PACK_UNPACK_BARRIER");
        MaybeDeviceZoneScopedN(qktv_barrier_split_profile, "FAP_QKTV_BARRIER_TOTAL");
        PACK((t6_semaphore_post<p_stall::STALL_PACK>(semaphore::PACK_DONE)));
        UNPACK((t6_semaphore_wait_on_zero<p_stall::STALL_SYNC>(semaphore::PACK_DONE)));
        UNPACK((t6_semaphore_get<>(semaphore::PACK_DONE)));
    };

    auto run_qktv_matmul_group = [&](uint32_t qktv_in0_index_offset, uint32_t w_q, uint32_t cur_h, uint32_t inner_dim) {
        DeviceZoneScopedN("FAP_COMPUTE_QKTV_MATMUL_PACK");
        ...
        for (uint32_t v_subblock = 0; v_subblock < qktv_v_num_subblocks; ++v_subblock) {
            {
                MaybeDeviceZoneScopedN(qktv_detail_profile, "FAP_COMPUTE_QKTV_V_SUBBLOCK");
                MaybeDeviceZoneScopedN(qktv_body_detail_profile, "FAP_COMPUTE_QKTV_V_SUBBLOCK");
                blocked_matmul_and_pack<false, vDHt, vDHt, qktv_body_detail_profile, qk_detail_profile_stage>(
                    cb_qkt_im,
                    cb_v_in,
                    out_cb,
    ...
    auto run_qktv_matmul_group_kt =
        [&](uint32_t qktv_in0_index_offset, uint32_t w_q, uint32_t cur_h, uint32_t kt_sub, uint32_t matmul_inner) {
            DeviceZoneScopedN("FAP_COMPUTE_QKTV_MATMUL_PACK");
            MaybeDeviceZoneScopedN(fa3_pipe_detail_profile, "FAP_FA3_PV_PREV_MATMUL");
            uint32_t v_index_offset = 0;
            if constexpr (!uniform_unpack_format) {
                reconfig_data_format(out_cb, cb_v_in, out_cb, cb_qkt_im);
            }
    ...
    {
        DeviceZoneScopedN("FAP_COMPUTE_WAIT_K");
        cb_wait_front(cb_kt_in, DHt * KT_stride);
    }

    {
        DeviceZoneScopedN("FAP_COMPUTE_QK_PHASE");
        for (uint32_t q_subblock = 0; q_subblock < q_num_subblocks; q_subblock++) {
            MaybeDeviceZoneScopedN(phase_timeline_profile, "FAP_TIMELINE_QK_ROW");
            {
                DeviceZoneScopedN("FAP_COMPUTE_QK_SETUP");
                {
                    DeviceZoneScopedN("FAP_COMPUTE_QK_WAIT_Q");
                    cb_wait_front(cb_q_in, q_wait_tiles);
                }
                kt_index_offset = 0;

                {
                    DeviceZoneScopedN("FAP_COMPUTE_QK_INIT");
                    const bool skip_qk_init = qk_init_is_primed && q_subblock == 0;
                    if (!skip_qk_init) {
                        if constexpr (!uniform_pack_format) {
                            MaybeDeviceZoneScopedN(qk_detail_profile_stage == 3, "FAP_COMPUTE_QK_INIT_PACK_DF");
                            pack_reconfig_data_format(cb_qkt_im);
                        }
                        if constexpr (!uniform_unpack_format) {
                            MaybeDeviceZoneScopedN(qk_detail_profile_stage == 3, "FAP_COMPUTE_QK_INIT_UNPACK_DF");
                            reconfig_data_format(cb_kt_in, cb_q_in);
                        }
                        {
                            MaybeDeviceZoneScopedN(qk_detail_profile_stage == 3, "FAP_COMPUTE_QK_INIT_MM_INIT");
                        ...
                        {
                            MaybeDeviceZoneScopedN(qk_detail_profile_stage == 3, "FAP_COMPUTE_QK_INIT_PACK_WIDTH");
                            configure_row_pack_width(cb_qkt_im, actual_sbw);
                        }
                    }
                }
            }
            auto run_prev_softmax = [&](uint32_t kt_subblock) {
                if (q_subblock == 0) {
                    return;
                }
                uint32_t prev_q_subblock = q_subblock - 1;
                {
                    DeviceZoneScopedN("FAP_COMPUTE_QK_SOFTMAX_EXP_SUM");
                    if constexpr (!uniform_unpack_format) {
                        reconfig_data_format(cb_kt_in, cb_qkt_im, cb_q_in, cb_qkt_im);
                    }
                    sub_exp_block_bcast_cols<scale_fp32, qk_softmax_profile_stage>(
                        cb_qkt_im,
                        cur.max,
                        cur.sum,
                        KT_stride,
            ...
            auto reinit_qk_matmul_after_softmax = [&]() {
                DeviceZoneScopedN("FAP_COMPUTE_QK_REINIT_AFTER_SOFTMAX");
                if constexpr (!uniform_pack_format) {
                    pack_reconfig_data_format(cb_qkt_im);
                }
                if constexpr (!uniform_unpack_format) {
            ...
            auto run_qk_matmul = [&](uint32_t kt_subblock) {
                DeviceZoneScopedN("FAP_COMPUTE_QK_MATMUL_PACK");
                if constexpr (!uniform_unpack_format) {
                    DeviceZoneScopedN("FAP_COMPUTE_QK_MATMUL_RECONFIG");
                ...
                {
                    DeviceZoneScopedN("FAP_COMPUTE_QK_MATMUL_BODY");
                    blocked_matmul_and_pack<
                        true,
                        KT_stride,
                        KT_stride,
                        qk_detail_profile_stage == 1,
                        qk_detail_profile_stage>(
                        cb_q_in,
                        cb_kt_in,
                        cb_qkt_im,
            ...
            for (uint32_t kt_subblock = 0; kt_subblock < kt_num_full_subblocks; ++kt_subblock) {
                MaybeDeviceZoneScopedN(phase_timeline_profile, "FAP_TIMELINE_QK_KT_STEP");
                if constexpr (qk_softmax_schedule == 1) {
                    if (q_subblock > 0 && kt_subblock > 0) {
                        reinit_qk_matmul_after_softmax();
                    }
                    run_qk_matmul(kt_subblock);
                    run_prev_softmax(kt_subblock);
                } else if constexpr (qk_softmax_schedule == 2) {
                    const bool is_final_kt_subblock = kt_subblock == kt_num_full_subblocks - 1;
                    if (q_subblock > 0 && !is_final_kt_subblock) {
                        if (kt_subblock > 0) {
                            reinit_qk_matmul_after_softmax();
                        }
                        run_qk_matmul(kt_subblock);
                    ...
                    if (q_subblock > 0) {
                        run_prev_softmax(kt_subblock);
                        if constexpr (fa3_pv_softmax_schedule) {
                            const uint32_t ready_group = q_subblock - 1;
                            if (ready_group < qktv_q_num_subblocks) {
                                if (kt_subblock == 0) {
                                    ensure_qktv_output_reserved();
                                    {
                                        DeviceZoneScopedN("FAP_COMPUTE_QKTV_WAIT_QKT");
                                        cb_wait_front(cb_qkt_im, (ready_group + 1) * qktv_in0_row_tiles);
                                    }
                                    ensure_qktv_v_ready();
                                }
                                {
                                    MaybeDeviceZoneScopedN(fa3_pipe_detail_profile, "FAP_FA3_HANDOFF_GAP");
                                    qktv_pack_unpack_barrier();
                                }
                                MaybeDeviceZoneScopedN(fa3_pipe_detail_profile, "FAP_FA3_SOFTMAX_PV_OVERLAP_CANDIDATE");
                                run_qktv_matmul_group_kt(
                                    ready_group * qktv_in0_row_tiles,
                                    ready_group,
                                    qktv_h,
                                    kt_subblock,
                                    actual_sbw);
                                if (kt_subblock == kt_num_full_subblocks - 1) {
                                    early_qktv_groups = ready_group + 1;
                                }
                            }
                        }
                        reinit_qk_matmul_after_softmax();
            ...
            if constexpr (ring_mode || is_causal_sdpa || use_padded_mask) {
                if ((is_causal_sdpa && apply_causal) || (apply_mask && lw_partial_tile_idx > 0)) {
                    DeviceZoneScopedN("FAP_COMPUTE_QK_MASK");
            ...
            {
                DeviceZoneScopedN("FAP_COMPUTE_QK_MAX_REDUCE");
            ...
            if constexpr (compute_pipeline_schedule == 1) {
                if (q_subblock > 0) {
                    const uint32_t ready_group = q_subblock - 1;
                    if (ready_group < qktv_q_num_subblocks) {
                        MaybeDeviceZoneScopedN(phase_timeline_profile, "FAP_TIMELINE_PARTIAL_HANDOFF");
                        ensure_qktv_output_reserved();
                        ensure_qktv_v_ready();
                        {
                            DeviceZoneScopedN("FAP_COMPUTE_QKTV_WAIT_QKT");
    ...
    // Rows 0..N-2 are softmax'd in-place; row N-1 has raw matmul output.
    {
        ...
        {
            DeviceZoneScopedN("FAP_COMPUTE_QKV_PHASE");
            MaybeDeviceZoneScopedN(first_output_handoff_profile, "FAP_FIRST_OUT_GROUP0_DRAIN_MATMUL");
            MaybeDeviceZoneScopedN(fa3_pipe_detail_profile, "FAP_FA3_GROUP0_DRAIN_PIPE");
            MaybeDeviceZoneScopedN(phase_timeline_profile, "FAP_TIMELINE_QKTV_DRAIN_LAST_ROW");
            ...
            if constexpr (compute_pipeline_schedule == 2) {
                ...
                for (uint32_t kt_sub = 0; kt_sub < kt_num_full_subblocks; ++kt_sub) {
                    ...
                    if (!group0_already_computed) {
                        MaybeDeviceZoneScopedN(qktv_detail_profile, "FAP_COMPUTE_QKTV_DRAIN_MATMUL_KT");
                        MaybeDeviceZoneScopedN(qktv_pipeline_detail_profile, "FAP_QKTV_PIPE_GROUP0_MATMUL_KT");
                        MaybeDeviceZoneScopedN(fa3_pipe_detail_profile, "FAP_FA3_PV_PREV_MATMUL");
                        uint32_t v_index_offset = 0;
                        if constexpr (!uniform_unpack_format) {
                            reconfig_data_format(out_cb, cb_v_in, out_cb, cb_qkt_im);
        ...
        if constexpr (first_output_handoff_profile) {
            DeviceZoneScopedN("FAP_FIRST_OUT_BARRIER_AFTER_GROUP0");
            qktv_pack_unpack_barrier();
        } else if constexpr (fa3_pipe_detail_profile) {
            DeviceZoneScopedN("FAP_FA3_HANDOFF_GAP");
            qktv_pack_unpack_barrier();
        } else {
            qktv_pack_unpack_barrier();
        }
        ...
        [[maybe_unused]] auto normalize_row = [&](uint32_t& pushed, uint32_t sbh) {
            DeviceZoneScopedN("FAP_COMPUTE_ROW_NORM");
            if constexpr (first_output_handoff_profile) {
                if (pushed == 0) {
                    DeviceZoneScopedN("FAP_FIRST_OUT_ROW_NORM");
            ...
            if constexpr (fa3_pipe_detail_profile) {
                if (pushed == 0) {
                    DeviceZoneScopedN("FAP_FA3_OUTPUT_PUSH");
                    cb_push_back(cur.sum, sbh);
                    cb_push_back(out_cb, sbh * vDHt);
                    normalize_row_streaming<profiling_enabled, vDHt, dst_size, uniform_pack_format>(
                        cur.sum, out_cb, cb_col_identity, cb_recip_scratch, cb_normalized_out, sbh);
        ...
        auto salad_correct_row = [&](uint32_t salad_row, uint32_t w_salad, uint32_t sbh) {
            PACK((llk_pack_reconfig_l1_acc(1)));
            auto run_salad_correct = [&]() {
                DeviceZoneScopedN("FAP_COMPUTE_SALAD_CORRECT");
            ...
            if constexpr (first_output_handoff_profile) {
                if (salad_row == 0) {
                    DeviceZoneScopedN("FAP_FIRST_OUT_SALAD_CORRECT");
                    run_salad_correct();
                } else {
                    run_salad_correct();
                }
        ...
        for (uint32_t q_subblock = 1; q_subblock < total_v_row_groups; ++q_subblock) {
            MaybeDeviceZoneScopedN(qktv_pipeline_detail_profile, "FAP_QKTV_PIPE_MAIN_GROUP");
            ...
            if (!is_first_iter && !(llk_prev_exp_after_pv_schedule && !is_remainder_iter)) {
                ...
                if constexpr (first_output_handoff_profile) {
                    if (q_subblock == 1) {
                        DeviceZoneScopedN("FAP_FIRST_OUT_EXP_MAX_DIFF");
                        run_exp_max_diff();
                    } else {
                        run_exp_max_diff();
                    }
            ...
            if constexpr (salad_first_handoff) {
            ...
            // must equal the actual tile-row offset). This must be computed after any early handoff
            // because pushed_rows changes the front-relative destination index.
            uint32_t w_q =
            ...
            const bool qktv_group_already_computed = q_subblock < early_qktv_groups;
            if (!qktv_group_already_computed) {
                ...
                if constexpr (first_output_handoff_profile) {
                    if (q_subblock == 1) {
                        DeviceZoneScopedN("FAP_FIRST_OUT_NEXT_GROUP_MATMUL");
                        run_current_group_matmul();
                    } else {
                        run_current_group_matmul();
                    }
                } else {
...
void sdpa_standard_v2(
    ...
    if constexpr (qk_first_body_warmup == 1) {
        qk_first_body_tiny_matmul_warmup<cb_identity_scale_in, cb_exp_max_diff>();
    }
    ...
    for (uint32_t q = 0; q < q_chunks_per_core; q++) {
        ...
        auto call_step = [&](auto profiling_tag,
            ...
            sdpa_inner_loop_step<
                ...
                qktv_h_limit,
                salad_first_handoff,
                qk_softmax_profile_stage,
                qk_softmax_schedule,
                qk_detail_profile_stage,
                qk_first_body_warmup,
                compute_pipeline_schedule>(
                prev,
                cur,
                is_last,
                is_first,
