...
#include "tt_metal/programming_examples/profiler/flash_attention_profile/kernels/q_chunk_remapping.hpp"

// Template-driven profiling for optional, copied-only detail zones.
// Disabled instantiations compile away instead of leaving runtime branches in
// the dataflow hot path.
#if defined(PROFILE_KERNEL)
template <bool Enabled, uint32_t timer_id>
struct FapMaybeProfileScope {
    inline __attribute__((always_inline)) FapMaybeProfileScope() {}
    inline __attribute__((always_inline)) ~FapMaybeProfileScope() {}
};
template <uint32_t timer_id>
struct FapMaybeProfileScope<true, timer_id> : kernel_profiler::profileScope<timer_id> {};

#define FapMaybeDeviceZoneScopedN(ENABLED, name)                                   \
    DO_PRAGMA(message(PROFILER_MSG_NAME(name)));                                   \
    auto constexpr fap_hash = kernel_profiler::Hash16_CT(PROFILER_MSG_NAME(name)); \
    FapMaybeProfileScope<ENABLED, fap_hash> fap_zone;
#else
#define FapMaybeDeviceZoneScopedN(ENABLED, name)
#endif
...
template <uint32_t tile_bytes, bool profile_q_read = false, typename ReaderType>
FORCE_INLINE void read_q_subblock(
    ...
    {
        FapMaybeDeviceZoneScopedN(profile_q_read, "FAP_READER_Q_RESERVE");
        cb_reserve_back(cb_id, sb_tiles);
    }
    const uint32_t base_write_ptr = get_write_ptr(cb_id);

    {
        FapMaybeDeviceZoneScopedN(profile_q_read, "FAP_READER_Q_NOC_READS");
        uint32_t barrier_count = 0;
    ...
    {
        FapMaybeDeviceZoneScopedN(profile_q_read, "FAP_READER_Q_READ_BARRIER");
        noc_async_read_barrier();
    }
    {
        FapMaybeDeviceZoneScopedN(profile_q_read, "FAP_READER_Q_PUSH");
    ...
    for (uint32_t rg = 0; rg < num_groups; ++rg) {
        ...
        const uint32_t tiles_this_group = rows_this_group * cols;
        {
            ...
            if constexpr (profile_writer_pipeline) {
                if (rg == 0) {
                    DeviceZoneScopedN("FAP_WRITER_FIRST_GROUP_WAIT_OUTPUT");
        ...
        uint32_t l1_read_addr = get_read_ptr(cb_out);
        {
            ...
            if constexpr (profile_writer_pipeline) {
                if (rg == 0) {
                    DeviceZoneScopedN("FAP_WRITER_FIRST_GROUP_STORE_TILES");
        ...
        // Flush THIS drain's writes (by trid) before pop so compute can safely reuse the L1 slot.
        {
            ...
            if constexpr (profile_writer_pipeline) {
                if (rg == 0) {
                    DeviceZoneScopedN("FAP_WRITER_FIRST_GROUP_STORE_FLUSH");
...
void write_block_row_grouped_with_group_ready(
    ...
    uint32_t tile_id = out_tile_id;
    uint32_t barrier_count = 0;
    drain_cb_row_grouped<profile_writer_pipeline>(
