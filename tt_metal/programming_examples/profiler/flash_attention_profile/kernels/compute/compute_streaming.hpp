 ...
 */
template <bool transpose, uint32_t in1_stride, uint32_t out_num_cols, bool profile_qk_body = false>
SDPA_NOINLINE void blocked_matmul_and_pack(
    ...
    {
        MaybeDeviceZoneScopedN(profile_qk_body, "FAP_COMPUTE_QK_BODY_TILE_REGS_ACQUIRE");
        tile_regs_acquire();
    }
    uint32_t dst_index = 0;
    ...
    {
        MaybeDeviceZoneScopedN(profile_qk_body, "FAP_COMPUTE_QK_BODY_MATMUL_LOOP");
    ...
    {
        MaybeDeviceZoneScopedN(profile_qk_body, "FAP_COMPUTE_QK_BODY_TILE_REGS_COMMIT");
        tile_regs_commit();
    }

    {
        MaybeDeviceZoneScopedN(profile_qk_body, "FAP_COMPUTE_QK_BODY_TILE_REGS_WAIT");
        tile_regs_wait();
    }
    if (!skip_pack_configure) {
        MaybeDeviceZoneScopedN(profile_qk_body, "FAP_COMPUTE_QK_BODY_PACK_CONFIG");
        configure_row_pack_width(out_cb, subblock_w);
    }
    {
        MaybeDeviceZoneScopedN(profile_qk_body, "FAP_COMPUTE_QK_BODY_PACK_ROWS");
        pack_contiguous_rows_nocfg(
            out_cb, row_subblock_idx * subblock_h, subblock_h, out_num_cols, out_col_offset, subblock_w);
    }
    if (trigger_reduce) {
        MaybeDeviceZoneScopedN(profile_qk_body, "FAP_COMPUTE_QK_BODY_REDUCE_SIGNAL");
        PACK((t6_semaphore_post<p_stall::NONE>(semaphore::FPU_SFPU)));
    }
    {
        MaybeDeviceZoneScopedN(profile_qk_body, "FAP_COMPUTE_QK_BODY_TILE_REGS_RELEASE");
        tile_regs_release();
    }
}
        ...
        for (uint32_t q_subblock = 0; q_subblock < q_num_subblocks; q_subblock++) {
            ...
            auto run_qk_matmul = [&](uint32_t kt_subblock) {
                ...
                bool kt_trigger_reduce = reduce_trigger && (kt_subblock == kt_num_full_subblocks - 1);
                {
                    ...
                    blocked_matmul_and_pack<true, KT_stride, KT_stride, true>(
                        cb_q_in,
                        cb_kt_in,
                        cb_qkt_im,
                        q_index_offset,
