...
ttsl::hash::hash_t SDPAOperation::compute_program_hash(const SDPAParams& attrs, const SDPAInputs& tensors) {
    ...
    const std::optional<uint32_t> qk_subblock_h_for_hash =
        attrs.qk_subblock_override.has_value() ? std::optional<uint32_t>{attrs.qk_subblock_override->first}
                                               : std::nullopt;
    const std::optional<uint32_t> qk_subblock_w_for_hash =
        attrs.qk_subblock_override.has_value() ? std::optional<uint32_t>{attrs.qk_subblock_override->second}
                                               : std::nullopt;
    operation::Hash hash = operation::hash_operation<SDPAOperation>(
        ...
        attrs.use_mla,
        static_cast<uint32_t>(attrs.pipeline_mode),
        attrs.streaming_output_depth,
        qk_subblock_h_for_hash,
        qk_subblock_w_for_hash,
        attrs.q_buffer_factor_override,
        attrs.dst_full_sync_override,
        attrs.qk_softmax_profile_stage,
        attrs.qk_softmax_schedule,
        attrs.qk_detail_profile_stage,
        attrs.q_reader_schedule,
        attrs.qk_first_body_warmup,
        attrs.compute_pipeline_schedule);
    return hash;
}
...
Tensor sdpa(
    ...
    SDPAPipelineMode pipeline_mode,
    uint32_t streaming_output_depth,
    ...
    uint32_t qk_softmax_profile_stage,
    uint32_t qk_softmax_schedule,
    uint32_t qk_detail_profile_stage,
    uint32_t q_reader_schedule,
    uint32_t qk_first_body_warmup,
    uint32_t compute_pipeline_schedule) {
    using OperationType = ttnn::prim::flash_attention_profile_llk_microflow_sdpa::SDPAOperation;
    return ttnn::device_operation::launch<OperationType>(
        OperationType::operation_attributes_t{
            ...
            .streaming_output_depth = streaming_output_depth,
            .qk_subblock_override = qk_subblock_override,
            .q_buffer_factor_override = q_buffer_factor_override,
            .dst_full_sync_override = dst_full_sync_override,
            .qk_softmax_profile_stage = qk_softmax_profile_stage,
            .qk_softmax_schedule = qk_softmax_schedule,
            .qk_detail_profile_stage = qk_detail_profile_stage,
            .q_reader_schedule = q_reader_schedule,
            .qk_first_body_warmup = qk_first_body_warmup,
            .compute_pipeline_schedule = compute_pipeline_schedule,
        },
