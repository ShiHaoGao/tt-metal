...
SDPAProgramFactory::cached_program_t SDPAProgramFactory::create(
    ...
    */
    ...
    const uint32_t q_buffer_factor = operation_attributes.q_buffer_factor_override.value_or((q_per_core > 1) ? 2 : 1);
    TT_FATAL(q_buffer_factor > 0 && q_buffer_factor <= 4, "q_buffer_factor override must be in [1, 4]");
    ...
    if (operation_attributes.qk_subblock_override.has_value()) {
        qk_out_subblock_h = operation_attributes.qk_subblock_override->first;
        qk_out_subblock_w = operation_attributes.qk_subblock_override->second;
        TT_FATAL(qk_out_subblock_h > 0 && qk_out_subblock_w > 0, "QK subblock override must be non-zero");
        TT_FATAL(
            qk_out_subblock_h * qk_out_subblock_w <= dst_size,
            "QK subblock override {}x{} exceeds dst register capacity {}",
            qk_out_subblock_h,
            qk_out_subblock_w,
            dst_size);
        TT_FATAL(
            Sq_chunk_t % qk_out_subblock_h == 0 && Sk_chunk_t % qk_out_subblock_w == 0,
            "QK subblock override {}x{} must divide Sq_chunk_t={} and Sk_chunk_t={}",
            qk_out_subblock_h,
            qk_out_subblock_w,
            Sq_chunk_t,
            Sk_chunk_t);
    ...
    const bool can_streaming_compute =
        can_use_streaming_compute(use_provided_mask, use_attention_sink, sliding_window_size, fp32_dest_acc_en);
    bool use_streaming_compute = can_streaming_compute;
    uint32_t streaming_out_subblock_h_limit = 2;
    uint32_t streaming_qktv_h_limit = UINT32_MAX;
    bool salad_first_handoff = false;
    switch (operation_attributes.pipeline_mode) {
        case SDPAPipelineMode::Auto:
            break;
        case SDPAPipelineMode::StreamH1:
            TT_FATAL(
                can_streaming_compute,
                "stream_h1 pipeline requires streaming compute eligibility: no user mask, no attention sink, no "
                "sliding window, and fp32 destination accumulation disabled");
            use_streaming_compute = true;
            streaming_out_subblock_h_limit = 1;
            break;
        case SDPAPipelineMode::QktvH1:
            TT_FATAL(
                can_streaming_compute,
                "qktv_h1 pipeline requires streaming compute eligibility: no user mask, no attention sink, no "
                "sliding window, and fp32 destination accumulation disabled");
            use_streaming_compute = true;
            streaming_out_subblock_h_limit = 1;
            streaming_qktv_h_limit = 1;
            break;
        case SDPAPipelineMode::SaladFirst:
            TT_FATAL(
                can_streaming_compute,
                "salad_first pipeline requires streaming compute eligibility: no user mask, no attention sink, no "
                "sliding window, and fp32 destination accumulation disabled");
            use_streaming_compute = true;
            salad_first_handoff = true;
            break;
        case SDPAPipelineMode::QktvH1SaladFirst:
            TT_FATAL(
                can_streaming_compute,
                "qktv_h1_salad_first pipeline requires streaming compute eligibility: no user mask, no attention sink, no "
                "sliding window, and fp32 destination accumulation disabled");
            use_streaming_compute = true;
            streaming_out_subblock_h_limit = 1;
            streaming_qktv_h_limit = 1;
            salad_first_handoff = true;
            break;
        case SDPAPipelineMode::NonStreaming:
            use_streaming_compute = false;
            break;
    }
    TT_FATAL(operation_attributes.streaming_output_depth > 0, "streaming output depth must be > 0");
    log_debug(
        tt::LogOp,
        "pipeline_mode: {} can_streaming_compute: {} use_streaming_compute: {} streaming_out_subblock_h_limit: {} streaming_qktv_h_limit: {} salad_first_handoff: {} streaming_output_depth: {}",
        static_cast<uint32_t>(operation_attributes.pipeline_mode),
        can_streaming_compute,
        use_streaming_compute,
        streaming_out_subblock_h_limit,
        streaming_qktv_h_limit,
        salad_first_handoff,
        operation_attributes.streaming_output_depth);
    ...
    auto [out_out_subblock_h, out_out_subblock_w] =
        detail::determine_largest_subblock_size(
            Sq_chunk_t, vDHt, dst_size, use_streaming_compute ? streaming_out_subblock_h_limit : UINT32_MAX);

    const uint32_t out_in0_num_subblocks = Sq_chunk_t / out_out_subblock_h;
    const uint32_t out_in1_num_subblocks = vDHt / out_out_subblock_w;
    const uint32_t out_num_blocks = Sk_chunk_t / out_in0_block_w;
    const uint32_t qktv_h = ttnn::transformer::sdpa::streaming_qktv_h(
        out_out_subblock_h, out_out_subblock_w, dst_size, Sq_chunk_t, streaming_qktv_h_limit);

    // Streaming: shrink cb_out to a 2-slot ping-pong (see sdpa_subblock_utils.hpp).
    if (use_streaming_compute) {
        out0_t = detail::streaming_cb_out_tiles(
            out_out_subblock_h,
            out_out_subblock_w,
            dst_size,
            Sq_chunk_t,
    ...
    reader_compile_time_args.push_back(operation_attributes.qk_detail_profile_stage);  // arg 32
    reader_compile_time_args.push_back(operation_attributes.q_reader_schedule);  // arg 33
    ...
    TT_FATAL(
        operation_attributes.qk_softmax_profile_stage <= 5,
        "qk_softmax_profile_stage must be in [0, 5], got {}",
        operation_attributes.qk_softmax_profile_stage);
    TT_FATAL(
        operation_attributes.qk_softmax_schedule <= 2,
    ...
    TT_FATAL(
        ...
        "q_reader_schedule must be in [0, 2], got {}",
        operation_attributes.q_reader_schedule);
    TT_FATAL(
        operation_attributes.qk_first_body_warmup <= 2,
    ...
    if (operation_attributes.compute_pipeline_schedule != 0) {
        ...
        TT_FATAL(
            qktv_h == qk_out_subblock_h,
            "compute pipeline experiments require qktv_h ({}) to equal qk_out_subblock_h ({})",
            qktv_h,
            qk_out_subblock_h);
    }
    ...
    std::vector<uint32_t> compute_compile_time_args = {
        ...
        streaming_qktv_h_limit,                       // arg 35
        static_cast<uint32_t>(salad_first_handoff),   // arg 36
        operation_attributes.qk_softmax_profile_stage, // arg 37
