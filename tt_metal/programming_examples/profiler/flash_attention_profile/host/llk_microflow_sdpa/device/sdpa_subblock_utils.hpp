...
namespace ttnn::prim::flash_attention_profile_llk_microflow_sdpa::detail {
...
// Streaming cb_out sizing: configurable slots of matmul row-groups (default 2 for
// the copied TTNN baseline). qktv_h comes from the shared helper in sdpa_streaming_qktv.hpp so host and
// kernel can't drift — see that header for the formula and rationale.
static inline uint32_t streaming_cb_out_tiles(
    uint32_t out_out_subblock_h,
    uint32_t out_out_subblock_w,
    uint32_t dst_size,
    uint32_t Sq_chunk_t,
    uint32_t vDHt,
    uint32_t depth,
    uint32_t qktv_h_limit = UINT32_MAX) {
    const uint32_t qktv_h = ttnn::transformer::sdpa::streaming_qktv_h(
        out_out_subblock_h, out_out_subblock_w, dst_size, Sq_chunk_t, qktv_h_limit);
    return depth * qktv_h * vDHt;
}
