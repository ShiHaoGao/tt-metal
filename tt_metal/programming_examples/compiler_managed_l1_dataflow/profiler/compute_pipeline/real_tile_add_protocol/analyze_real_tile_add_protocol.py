...
ZONE_RE = re.compile(
    r"^RTADD_(?P<mode>CB|STATIC_RUNTIME|STATIC_COMPILETIME|STATIC_STREAMREG|STATIC_STREAMREG_CBREGS)_"
    r"(?P<stage>READER|WRITER|COMPUTE_UNPACK|COMPUTE_MATH|COMPUTE_PACK)$"
)
CASE_RE = re.compile(
    r"real_tile_add_protocol: tiles=(?P<tiles>\d+), tile_size=(?P<tile_size>\d+), "
    r"num_pages=(?P<num_pages>\d+), repeats=(?P<repeats>\d+), "
    r"core=\((?P<core_x>\d+), (?P<core_y>\d+)\)"
    r"(?:, core_grid=\((?P<core_grid_x>\d+), (?P<core_grid_y>\d+)\))?, "
    r"serialized_static=(?P<serialized_static>true|false)"
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize real_tile_add_protocol host and device-profiler output.")
    parser.add_argument("--host-log", type=Path, required=True, help="Log captured from real_tile_add_protocol stdout.")
    parser.add_argument(
        "--device-csv",
        type=Path,
    ...
    parser.add_argument("--host-results-csv", type=Path, help="Optional output CSV for raw host rows.")
    parser.add_argument("--host-summary-csv", type=Path, help="Optional output CSV for host summary rows.")
        ...
        for line in log_file:
            ...
            if case_match:
                metadata = {
                    ...
                    "core_grid_x": int(case_match.group("core_grid_x") or 1),
                    "core_grid_y": int(case_match.group("core_grid_y") or 1),
                    "serialized_static": case_match.group("serialized_static") == "true",
                }
                continue
...
def summarize_host(rows):
    ...
    for row in rows:
        key = (row["tiles"], row["num_pages"], row["core_grid_x"], row["core_grid_y"], row["mode"])
        grouped[key].append(row)

    summaries = []
    for (tiles, num_pages, core_grid_x, core_grid_y, mode), samples in sorted(grouped.items()):
