...
#define SPM_CASE_PREFIX \
    SPM_OP_PREFIX "_T" SPM_STRINGIZE(BENCH_ITERATIONS) "_C" SPM_STRINGIZE(BENCH_CHAIN_DEPTH) "_S" SPM_STRINGIZE(BENCH_NUM_PAGES)
#else
#define SPM_CASE_PREFIX SPM_OP_PREFIX
#endif
#define SPM_ZONE_STATIC_COMPILETIME_READER SPM_CASE_PREFIX "_STATIC_COMPILETIME_READER"
#define SPM_ZONE_STATIC_RUNTIME_READER SPM_CASE_PREFIX "_STATIC_RUNTIME_READER"
#define SPM_ZONE_STATIC_SERIALIZED_READER SPM_CASE_PREFIX "_STATIC_SERIALIZED_READER"
#define SPM_ZONE_STATIC_STREAMREG_READER SPM_CASE_PREFIX "_STATIC_STREAMREG_READER"
#define SPM_ZONE_STATIC_STREAMREG_CBREGS_READER SPM_CASE_PREFIX "_STATIC_STREAMREG_CBREGS_READER"
#define SPM_ZONE_STATIC_STREAMREG_CBREGS_COMPILETIME_READER \
    SPM_CASE_PREFIX "_STATIC_STREAMREG_CBREGS_COMPILETIME_READER"
#define SPM_ZONE_CB_READER SPM_CASE_PREFIX "_CB_READER"
...
void kernel_main() {
    ...
    const uint32_t input_ready_sem_addr = get_arg_val<uint32_t>(7);
    const uint32_t input_consumed_sem_addr = get_arg_val<uint32_t>(8);
    const uint32_t output_ready_sem_addr = get_arg_val<uint32_t>(9);
    const uint32_t output_consumed_sem_addr = get_arg_val<uint32_t>(10);
    ...
    DEVICE_PRINT("spm reader start value={} slots={}\n", BENCH_PROTOCOL_START_VALUE, num_pages);
#endif

#if BENCH_USE_STREAM_REG_CBREGS && BENCH_USE_COMPILE_TIME_ARGS
    DeviceZoneScopedN(SPM_ZONE_STATIC_STREAMREG_CBREGS_COMPILETIME_READER);
#elif BENCH_USE_COMPILE_TIME_ARGS
    DeviceZoneScopedN(SPM_ZONE_STATIC_COMPILETIME_READER);
#elif BENCH_SERIAL_STATIC_PROTOCOL
    DeviceZoneScopedN(SPM_ZONE_STATIC_SERIALIZED_READER);
#elif BENCH_USE_STREAM_REG_CBREGS
    DeviceZoneScopedN(SPM_ZONE_STATIC_STREAMREG_CBREGS_READER);
#elif BENCH_USE_STREAM_REG_SYNC
    DeviceZoneScopedN(SPM_ZONE_STATIC_STREAMREG_READER);
#else
    DeviceZoneScopedN(SPM_ZONE_STATIC_RUNTIME_READER);
#endif
    for (uint32_t i = 0; i < iterations; ++i) {
...
#if BENCH_USE_STREAM_REG_SYNC
            wait_min_stream(BENCH_STREAM_REG_OUTPUT_CONSUMED_STREAM_ID, prev + 1);
#else
        ...
        noc_async_read(get_noc_addr(i, src0_addrgen), dst0_l1_addr, page_size);
        noc_async_read(get_noc_addr(i, src1_addrgen), dst1_l1_addr, page_size);
...
#if BENCH_TRACE_STATIC_PROTOCOL
        if (i < 4) {
            DEVICE_PRINT("spm reader ready i={} slot={} gen={}\n", i, slot, generation);
        }
#endif
    }
#else
