...
from analyze_ttnn_binary_ng_row_bcast_protocol import (
    parse_device_zones,
    parse_host_results,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run TTNN binary-ng row-bcast protocol profiler cases.")
    parser.add_argument(
        "--exe",
        type=Path,
        default=Path("build_Release/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_binary_ng_row_bcast_protocol"),
    )
    parser.add_argument("--out-dir", type=Path, default=Path("/tmp/ttnn_binary_ng_row_bcast_protocol_cases"))
    parser.add_argument("--ops", nargs="+", default=["add"], choices=["add"])
    parser.add_argument("--tiles", type=int, nargs="+", default=[1024, 4096, 16384])
    parser.add_argument("--width-tiles", type=int, nargs="+", default=[8])
...
def run_case(args, op, tiles, width_tiles, num_pages, mode):
    case_name = f"{op}_T{tiles}_W{width_tiles}_S{num_pages}_{mode}"
...
def build_critical_comparisons(critical_rows):
    ...
    for row in critical_rows:
        key = (row["op"], row["tiles"], row["width_tiles"], row["num_pages"], row["core_grid_x"], row["core_grid_y"])
    ...
    for key, modes in sorted(grouped.items()):
        ...
        max_tiles_per_core = max(1, (key[1] + active_cores - 1) // active_cores)
            ...
            comparisons.append(
                {
                    ...
                    "delta_cycles_per_tile": delta / max(1, key[1]),
...
def build_host_comparisons(host_summary_rows):
    ...
    for row in host_summary_rows:
        grouped[(row["op"], row["tiles"], row["width_tiles"], row["num_pages"], row["core_grid_x"], row["core_grid_y"])][row["mode"]] = row
            ...
            comparisons.append(
                {
                    ...
                    "delta_us_per_tile": delta / max(1, key[1]),
...
def main():
    ...
    for op in args.ops:
        for tiles in args.tiles:
            for width_tiles in args.width_tiles:
                if tiles % width_tiles != 0:
                    raise ValueError(f"tiles={tiles} must be divisible by width_tiles={width_tiles}")
                for num_pages in args.num_pages:
                    for mode in args.modes:
                        print(f"running op={op} tiles={tiles} width_tiles={width_tiles} pages={num_pages} mode={mode}", flush=True)
                        host_rows, host_summary, zone_rows, critical_rows = run_case(
                            args, op, tiles, width_tiles, num_pages, mode
                        )
