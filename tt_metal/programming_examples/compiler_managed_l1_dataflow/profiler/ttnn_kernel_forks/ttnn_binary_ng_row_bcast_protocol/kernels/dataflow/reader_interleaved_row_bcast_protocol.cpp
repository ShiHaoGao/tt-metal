...
#ifndef FILL_TILE_WITH_FIRST_ROW
#define FILL_TILE_WITH_FIRST_ROW fill_tile_with_first_row_bfloat16
#endif
...
inline void set_static_write_base(uint32_t cb_id, uint32_t l1_addr) {
    get_local_cb_interface(cb_id).fifo_wr_ptr = l1_addr;
...
void kernel_main() {
    ...
    DeviceZoneScopedN("TBNGRB_STATIC_STREAMREG_CBREGS_COMPILETIME_READER");
#elif BENCH_USE_STREAM_REG_SYNC
    DeviceZoneScopedN("TBNGRB_STATIC_STREAMREG_READER");
#elif BENCH_USE_STREAM_REG_CBREGS
    DeviceZoneScopedN("TBNGRB_STATIC_STREAMREG_CBREGS_READER");
#else
    DeviceZoneScopedN("TBNGRB_STATIC_RUNTIME_READER");
#endif
                ...
                for (uint32_t c = start_c; c < C && num_tiles_read < dst_num_tiles; ++c, start_th = 0) {
                    for (uint32_t th = start_th; th < Ht && num_tiles_read < dst_num_tiles; ++th) {
                        for (uint32_t tw = start_tw; tw < end_tw && num_tiles_read < dst_num_tiles;
                            ...
                            set_static_write_base(kCbSrc0, dst0_l1_addr);
                            FILL_TILE_WITH_FIRST_ROW(kCbSrc0);
#endif
#if SRC_BCAST_B && !BCAST_LLK
                            set_static_write_base(kCbSrc1, dst1_l1_addr);
                            FILL_TILE_WITH_FIRST_ROW(kCbSrc1);
#endif
#if BENCH_USE_STREAM_REG_SYNC
...
#else
    DeviceZoneScopedN("TBNGRB_CB_READER");
