...
constexpr std::string_view kReaderKernel =
    "tt_metal/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_kernel_forks/ttnn_binary_ng_row_bcast_protocol/kernels/dataflow/"
    "reader_interleaved_row_bcast_protocol.cpp";
constexpr std::string_view kWriterKernel =
    "tt_metal/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_kernel_forks/ttnn_binary_ng_row_bcast_protocol/kernels/dataflow/"
    "writer_interleaved_row_bcast_protocol.cpp";
constexpr std::string_view kComputeKernel =
    "tt_metal/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_kernel_forks/ttnn_binary_ng_row_bcast_protocol/kernels/compute/"
    "eltwise_binary_row_bcast_protocol.cpp";
...
struct CoreWork {
    ...
    std::shared_ptr<Buffer> bcast_ring_buffer;
...
bool uses_compile_time_protocol_args(Mode mode) { return mode == Mode::StaticStreamRegCbRegsCompileTime; }
...
void print_usage(const char* argv0) {
    fmt::print(
        ...
        "[--op=add] [--tiles=N] [--width-tiles=N] [--num-pages=N] [--repeats=N] "
...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            ...
            options.op = std::string(arg.substr(std::string_view("--op=").size()));
    ...
    if (options.width_tiles == 0 || options.tiles % options.width_tiles != 0) {
        throw std::invalid_argument("--width-tiles must be greater than zero and divide --tiles");
    }
    if (options.op != "add") {
        throw std::invalid_argument("--op currently supports add only; the LLK SFPU row-bcast path is tracked separately");
    }
...
std::vector<bfloat16> make_input(uint32_t tiles, uint32_t salt) {
    ...
    for (uint32_t tile = 0; tile < tiles; ++tile) {
        const uint32_t row_value_seed = (tile * 131u + salt * 53u) % 97u;
        for (uint32_t i = 0; i < kTileElements; ++i) {
            const uint32_t col = i % tt::constants::TILE_WIDTH;
            const uint32_t pattern =
                salt == 1 ? row_value_seed + col : ((tile * 131u + i * 17u + salt * 53u) % 96u) + 1u;
...
std::map<std::string, std::string> compute_defines(
    ...
    defines.insert({
        ...
        {"BCAST_INPUT", "0"},
        {"BINARY_OP", "add_tiles"},
        {"BINARY_OP_TYPE", "EltwiseBinaryType::ELWADD"},
...
void create_circular_buffers(
    ...
    const std::shared_ptr<Buffer>& bcast_ring_buffer,
    ...
    CreateCircularBuffer(program, core, make_cb_config(CBIndex::c_5, bcast_ring_buffer));
}
...
bool validate_result(
    ...
    for (size_t i = 0; i < src0.size(); ++i) {
        const uint32_t tile = static_cast<uint32_t>(i / kTileElements);
        const uint32_t in_tile_element = static_cast<uint32_t>(i % kTileElements);
        const uint32_t word = in_tile_element / 2;
        const uint32_t lane = in_tile_element % 2;
        const uint32_t face = word / 128;
        const uint32_t row_bcast_word = (face == 0 || face == 2) ? (word % 8) : (128 + (word % 8));
        const uint32_t row_bcast_element = row_bcast_word * 2 + lane;
        const uint32_t src0_tile = tile % options.width_tiles;
        const float lhs =
            static_cast<float>(src0[static_cast<size_t>(src0_tile) * kTileElements + row_bcast_element]);
        ...
        if (error > 3e-1f) {
            ...
            if (printed < 5) {
                fmt::print(
                    stderr,
                    "Mismatch at element {}: expected {}, got {}, lhs={}, rhs={}, error={}\n",
                    i,
                    expected,
                    actual,
                    lhs,
...
std::vector<uint32_t> writer_runtime_args(const Options& options, const CoreWork& work, uint32_t dst_dram_addr) {
    return {
        ...
        1,
        options.width_tiles,
        height_tiles(options) * options.width_tiles,
        1,
        0};
}
...
RunResult run_one(
    ...
    if (uses_stream_reg_sync(mode)) {
        throw std::invalid_argument(
            "static-streamreg is disabled for ttnn_binary_ng_row_bcast_protocol: the old idle-stream scratch-register "
    ...
    const uint32_t payload_bytes = payload_size_bytes(options.tiles);
    ...
    if (is_static_mode(mode)) {
        ...
        for (auto& work : core_work) {
            ...
            work.bcast_ring_buffer = create_core_local_l1_buffer(mesh_device, work.core, ring_bytes, kTileSizeBytes);
            ...
            detail::WriteToBuffer(work.bcast_ring_buffer, zero_ring_buffer);
    ...
    for (const auto& work : core_work) {
        create_circular_buffers(
            ...
            mode,
            options,
            work.src0_ring_buffer,
            work.src1_ring_buffer,
            work.bcast_ring_buffer,
            work.dst_ring_buffer);
    }
    ...
    for (const auto& work : core_work) {
        std::vector<uint32_t> reader_args = reader_runtime_args(options, work, src0_dram_addr, src1_dram_addr);
    ...
    float max_abs_error = 0.0f;
    const bool ok = validate_result(options, src0, src1, result, &max_abs_error);
    return {
        .mode = mode,
        .repeat = repeat,
        .enqueue_finish_us =
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        fmt::print(
            "ttnn_binary_ng_row_bcast_protocol: op={}, tiles={}, width_tiles={}, height_tiles={}, tile_size={}, "
            "num_pages={}, repeats={}, core=({}, {}), core_grid=({}, {}), num_tiles_per_cycle={}\n",
            options.op,
            options.tiles,
            options.width_tiles,
            height_tiles(options),
