...
ZONE_RE = re.compile(
    r"^TBNGRB_(?P<mode>CB|STATIC_RUNTIME|STATIC_STREAMREG|STATIC_STREAMREG_CBREGS|STATIC_STREAMREG_CBREGS_COMPILETIME)_"
    r"(?P<stage>READER|WRITER|COMPUTE_UNPACK|COMPUTE_MATH|COMPUTE_PACK)$"
)
CASE_RE = re.compile(
    r"ttnn_binary_ng_row_bcast_protocol: op=(?P<op>[^,]+), tiles=(?P<tiles>\d+), "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize TTNN binary-ng row-bcast protocol profiler output.")
...
def summarize_host(rows):
    ...
    for row in rows:
        key = (row["op"], row["tiles"], row["width_tiles"], row["num_pages"], row["core_grid_x"], row["core_grid_y"], row["mode"])
        grouped[key].append(row)

    summaries = []
    for (op, tiles, width_tiles, num_pages, core_grid_x, core_grid_y, mode), samples in sorted(grouped.items()):
