...
STAGE_LABELS = {
    "READER": "reader",
    "WRITER": "writer",
    "COMPUTE_INPUT_UNTILIZE": "compute-input-untilize",
...
CASE_RE = re.compile(
    r"ttnn_paged_update_cache_protocol: users=(?P<users>\d+), kv_heads=(?P<kv_heads>\d+), "
    r"head_dim=(?P<head_dim>\d+), Wt=(?P<Wt>\d+), block_size=(?P<block_size>\d+), "
    r"block_size_t=(?P<block_size_t>\d+), max_seq_len=(?P<max_seq_len>\d+), "
    r"max_blocks_per_seq=(?P<max_blocks_per_seq>\d+), cache_idx=(?P<cache_idx>\d+), "
    r"per_user_stride=(?P<per_user_stride>\d+), cache_tiles=(?P<cache_tiles>\d+), "
    r"ring_rows=(?P<num_pages>\d+), repeats=(?P<repeats>\d+), "
    r"core=\((?P<core_x>\d+), (?P<core_y>\d+)\), active_cores=(?P<active_cores>\d+)"
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="汇总 ttnn_paged_update_cache_protocol profiler 输出。")
...
def summarize_host(rows):
    ...
    for row in rows:
        grouped[case_key(row) + (row["mode"],)].append(row)

    summaries = []
    for key, samples in sorted(grouped.items()):
        users, kv_heads, head_dim, block_size, max_seq_len, cache_idx, stride, num_pages, active_cores, mode = key
...
def parse_zone_name(zone):
    ...
    if not match:
        return None
    return {"mode": MODE_LABELS[match.group("mode")], "stage": STAGE_LABELS[match.group("stage")]}
...
def parse_device_zones(path):
    ...
    for row in rows[header_index + 1 :]:
        if len(row) <= max(time_col, zone_col, phase_col, core_x_col, core_y_col, risc_col):
            continue
        metadata = parse_zone_name(row[zone_col].strip())
        if metadata is None:
            continue
        key = (row[zone_col].strip(), row[core_x_col].strip(), row[core_y_col].strip(), row[risc_col].strip())
