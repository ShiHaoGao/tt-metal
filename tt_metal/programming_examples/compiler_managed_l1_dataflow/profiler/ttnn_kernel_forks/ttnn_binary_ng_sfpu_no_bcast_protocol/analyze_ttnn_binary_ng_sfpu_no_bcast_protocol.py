...
ZONE_RE = re.compile(
    r"^TBNG_(?P<mode>CB|STATIC_RUNTIME|STATIC_STREAMREG|STATIC_STREAMREG_CBREGS|STATIC_STREAMREG_CBREGS_COMPILETIME)_"
    r"(?P<stage>READER|WRITER|COMPUTE_UNPACK|COMPUTE_MATH|COMPUTE_PACK)$"
)
CASE_RE = re.compile(
    r"ttnn_binary_ng_sfpu_no_bcast_protocol: op=(?P<op>\S+), "
    r"tiles=(?P<tiles>\d+), tile_size=(?P<tile_size>\d+), "
    r"num_pages=(?P<num_pages>\d+), repeats=(?P<repeats>\d+), "
    r"core=\((?P<core_x>\d+), (?P<core_y>\d+)\), "
    r"core_grid=\((?P<core_grid_x>\d+), (?P<core_grid_y>\d+)\), "
    r"num_tiles_per_cycle=(?P<num_tiles_per_cycle>\d+)"
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize TTNN binary-ng SFPU no-bcast protocol profiler output.")
        ...
        for line in log_file:
            ...
            if case_match:
                metadata = {
                    ...
                    "num_tiles_per_cycle": int(case_match.group("num_tiles_per_cycle")),
                }
                continue
...
def summarize_host(rows):
    ...
    for row in rows:
        key = (row.get("op", "div"), row["tiles"], row["num_pages"], row["core_grid_x"], row["core_grid_y"], row["mode"])
        grouped[key].append(row)

    summaries = []
    for (op, tiles, num_pages, core_grid_x, core_grid_y, mode), samples in sorted(grouped.items()):
