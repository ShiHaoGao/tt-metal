...
from analyze_ttnn_binary_ng_sfpu_no_bcast_protocol import (
    parse_device_zones,
    parse_host_results,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run TTNN binary-ng SFPU no-bcast protocol profiler cases.")
    parser.add_argument(
        "--exe",
        type=Path,
        default=Path(
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("/tmp/ttnn_binary_ng_sfpu_no_bcast_protocol_cases"))
...
def run_case(args, tiles, num_pages, mode):
    case_name = f"div_T{tiles}_S{num_pages}_{mode}"
...
def build_critical_comparisons(critical_rows):
    ...
    for row in critical_rows:
        key = (row.get("op", "div"), row["tiles"], row["num_pages"], row["core_grid_x"], row["core_grid_y"])
...
def build_host_comparisons(host_summary_rows):
    ...
    for row in host_summary_rows:
        grouped[(row.get("op", "div"), row["tiles"], row["num_pages"], row["core_grid_x"], row["core_grid_y"])][
            row["mode"]
        ] = row
