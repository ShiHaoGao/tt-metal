...
constexpr std::string_view kReaderKernel =
    "tt_metal/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_kernel_forks/ttnn_binary_ng_no_bcast_protocol/kernels/dataflow/"
...
constexpr std::string_view kComputeKernel =
    "tt_metal/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_kernel_forks/ttnn_binary_ng_no_bcast_protocol/kernels/compute/"
    "eltwise_binary_no_bcast_protocol.cpp";
...
std::map<std::string, std::string> reader_defines(Mode mode, uint32_t protocol_start_value) {
    auto defines = protocol_defines(mode, protocol_start_value);
...
RunResult run_one(
    ...
    if (uses_stream_reg_sync(mode)) {
        throw std::invalid_argument(
            "static-streamreg is disabled for ttnn_binary_ng_no_bcast_protocol: the old idle-stream scratch-register "
    ...
    for (const auto& work : core_work) {
        ...
        if (is_static_mode(mode)) {
            ...
            reader_args.insert(
                reader_args.end(), {src0_ring_addr, src1_ring_addr, kTileSizeBytes, options.num_pages, protocol_start_addr});
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        fmt::print(
            "ttnn_binary_ng_no_bcast_protocol: tiles={}, tile_size={}, num_pages={}, repeats={}, core=({}, {}), "
