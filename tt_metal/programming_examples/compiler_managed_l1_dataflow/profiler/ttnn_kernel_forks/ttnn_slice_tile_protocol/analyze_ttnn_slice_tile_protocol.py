...
ZONE_RE = re.compile(r"^TTSL_(?P<mode>CB|STATIC_RUNTIME|STATIC_STREAMREG_CBREGS)_(?P<stage>READER|WRITER)$")
CASE_RE = re.compile(
    r"ttnn_slice_tile_protocol: input_height_tiles=(?P<input_height_tiles>\d+), "
    r"input_width_tiles=(?P<input_width_tiles>\d+), output_height_tiles=(?P<output_height_tiles>\d+), "
    r"output_width_tiles=(?P<output_width_tiles>\d+), start_height_tile=(?P<start_height_tile>\d+), "
    r"start_width_tile=(?P<start_width_tile>\d+), batches=(?P<batches>\d+), "
    r"output_tiles=(?P<output_tiles>\d+), tile_size=(?P<tile_size>\d+), "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize TTNN slice tile protocol profiler output.")
...
def parse_host_results(path):
    ...
    with path.open() as log_file:
        for line in log_file:
            ...
            if case_match:
                metadata = {
                    "input_height_tiles": int(case_match.group("input_height_tiles")),
                    "input_width_tiles": int(case_match.group("input_width_tiles")),
                    "output_height_tiles": int(case_match.group("output_height_tiles")),
                    "output_width_tiles": int(case_match.group("output_width_tiles")),
                    "start_height_tile": int(case_match.group("start_height_tile")),
                    "start_width_tile": int(case_match.group("start_width_tile")),
                    "batches": int(case_match.group("batches")),
