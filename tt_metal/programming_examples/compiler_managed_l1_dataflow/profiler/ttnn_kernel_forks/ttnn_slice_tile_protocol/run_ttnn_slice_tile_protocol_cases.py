...
from analyze_ttnn_slice_tile_protocol import (
    parse_device_zones,
    parse_host_results,
...
def parse_shape(value):
    ...
    if len(parts) == 4:
        input_height, input_width, output_height, output_width = parts
        start_height = 0
        start_width = 0
        batches = 1
    elif len(parts) == 6:
        input_height, input_width, output_height, output_width, start_height, start_width = parts
        batches = 1
    elif len(parts) == 7:
        input_height, input_width, output_height, output_width, start_height, start_width, batches = parts
    else:
        raise argparse.ArgumentTypeError(
            "shape must be IHxIWxOHxOW, IHxIWxOHxOWxSHxSW, or IHxIWxOHxOWxSHxSWxB"
        )
    try:
        values = tuple(
            int(part)
            for part in (input_height, input_width, output_height, output_width, start_height, start_width, batches)
        )
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run TTNN slice tile protocol profiler cases.")
    parser.add_argument(
        "--exe",
        type=Path,
        default=Path("build_Release/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_slice_tile_protocol"),
    )
    parser.add_argument("--out-dir", type=Path, default=Path("/tmp/ttnn_slice_tile_protocol_cases"))
    parser.add_argument(
        "--shapes",
        type=parse_shape,
        nargs="+",
...
def run_case(args, shape, num_pages, mode):
    input_height, input_width, output_height, output_width, start_height, start_width, batches = shape
    output_tiles = output_height * output_width * batches
    case_name = (
        f"I{input_height}x{input_width}_O{output_height}x{output_width}_"
        f"S{start_height}x{start_width}_B{batches}_P{num_pages}_{mode}"
    )
    ...
    command = [
        str(args.exe),
        f"--mode={mode}",
        f"--input-height-tiles={input_height}",
        f"--input-width-tiles={input_width}",
        f"--output-height-tiles={output_height}",
        f"--output-width-tiles={output_width}",
        f"--start-height-tile={start_height}",
        f"--start-width-tile={start_width}",
        f"--batches={batches}",
    ...
    common = {
        "case": case_name,
        "input_height_tiles": input_height,
        "input_width_tiles": input_width,
        "output_height_tiles": output_height,
        "output_width_tiles": output_width,
        "start_height_tile": start_height,
        "start_width_tile": start_width,
        "batches": batches,
...
def build_critical_comparisons(critical_rows):
    ...
    for row in critical_rows:
        key = (
            row["input_height_tiles"],
            row["input_width_tiles"],
            row["output_height_tiles"],
            row["output_width_tiles"],
            row["start_height_tile"],
            row["start_width_tile"],
            row["batches"],
            row["output_tiles"],
            row["num_pages"],
            row["core_grid_x"],
    ...
    for key, modes in sorted(grouped.items()):
        ...
        max_tiles_per_core = max(1, (key[7] + active_cores - 1) // active_cores)
        for static_mode in ["static-runtime", "static-streamreg-cbregs"]:
            ...
            comparisons.append(
                {
                    "input_height_tiles": key[0],
                    "input_width_tiles": key[1],
                    "output_height_tiles": key[2],
                    "output_width_tiles": key[3],
                    "start_height_tile": key[4],
                    "start_width_tile": key[5],
                    "batches": key[6],
                    "output_tiles": key[7],
                    "num_pages": key[8],
                    "core_grid_x": key[9],
                    ...
                    "delta_cycles_per_output_tile": delta / max(1, key[7]),
            ...
            comparisons.append(
                {
                    ...
                    "delta_us_per_output_tile": delta / max(1, key[7]),
    ...
    for shape in args.shapes:
        for num_pages in args.num_pages:
            for mode in args.modes:
                print(f"running shape={shape} pages={num_pages} mode={mode}", flush=True)
                host_rows, host_summary, zone_rows, critical_rows = run_case(args, shape, num_pages, mode)
