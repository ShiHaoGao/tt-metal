...
constexpr auto kCbIn = tt::CBIndex::c_0;
constexpr auto kCbOut = tt::CBIndex::c_16;
...
#ifdef TRISC_PACK
    TTI_STALLWAIT(p_stall::STALL_THCON, p_stall::PACK);
    set_stream_sync(BENCH_STREAM_REG_INPUT_CONSUMED_STREAM_ID, value);
...
void kernel_main() {
    ...
    transpose_wh_init(kCbIn, kCbOut);
...
#else
    const uint32_t src_ring_addr = get_arg_val<uint32_t>(1);
    ...
    const uint32_t protocol_start_sem_addr = get_arg_val<uint32_t>(5);
#endif

#if BENCH_USE_STREAM_REG_SYNC
    wait_equal_stream(BENCH_STREAM_REG_START_STREAM_ID, BENCH_PROTOCOL_START_VALUE);
#else
    volatile tt_reg_ptr uint32_t* input_ready_reg = reg_ptr_from_cb(kCbIn, true);
    volatile tt_reg_ptr uint32_t* input_consumed_reg = reg_ptr_from_cb(kCbIn, false);
    ...
    DeviceZoneScopedN("TTWH_STATIC_STREAMREG_CBREGS_COMPILETIME_COMPUTE_UNPACK");
#elif defined(TRISC_MATH)
    DeviceZoneScopedN("TTWH_STATIC_STREAMREG_CBREGS_COMPILETIME_COMPUTE_MATH");
#elif defined(TRISC_PACK)
    DeviceZoneScopedN("TTWH_STATIC_STREAMREG_CBREGS_COMPILETIME_COMPUTE_PACK");
#endif
#elif BENCH_USE_STREAM_REG_SYNC
#if defined(TRISC_UNPACK)
    DeviceZoneScopedN("TTWH_STATIC_STREAMREG_COMPUTE_UNPACK");
#elif defined(TRISC_MATH)
    DeviceZoneScopedN("TTWH_STATIC_STREAMREG_COMPUTE_MATH");
#elif defined(TRISC_PACK)
    DeviceZoneScopedN("TTWH_STATIC_STREAMREG_COMPUTE_PACK");
#endif
#elif BENCH_USE_STREAM_REG_CBREGS
#if defined(TRISC_UNPACK)
    DeviceZoneScopedN("TTWH_STATIC_STREAMREG_CBREGS_COMPUTE_UNPACK");
#elif defined(TRISC_MATH)
    DeviceZoneScopedN("TTWH_STATIC_STREAMREG_CBREGS_COMPUTE_MATH");
#elif defined(TRISC_PACK)
    DeviceZoneScopedN("TTWH_STATIC_STREAMREG_CBREGS_COMPUTE_PACK");
#endif
#else
#if defined(TRISC_UNPACK)
    DeviceZoneScopedN("TTWH_STATIC_RUNTIME_COMPUTE_UNPACK");
#elif defined(TRISC_MATH)
    DeviceZoneScopedN("TTWH_STATIC_RUNTIME_COMPUTE_MATH");
#elif defined(TRISC_PACK)
    DeviceZoneScopedN("TTWH_STATIC_RUNTIME_COMPUTE_PACK");
#endif
#endif
...
#ifdef TRISC_UNPACK
#if BENCH_USE_STREAM_REG_SYNC
        wait_min_stream(BENCH_STREAM_REG_INPUT_READY_STREAM_ID, generation);
#else
        wait_min_reg(input_ready_reg, generation);
#endif
#endif
        set_static_read_base(kCbIn, src_l1_addr);

        tile_regs_acquire();
        transpose_wh_tile(kCbIn, 0, 0);
        tile_regs_commit();
...
#else
#if defined(TRISC_UNPACK)
    DeviceZoneScopedN("TTWH_CB_COMPUTE_UNPACK");
#elif defined(TRISC_MATH)
    DeviceZoneScopedN("TTWH_CB_COMPUTE_MATH");
#elif defined(TRISC_PACK)
    DeviceZoneScopedN("TTWH_CB_COMPUTE_PACK");
#endif

    CircularBuffer cb_in(kCbIn);
    CircularBuffer cb_out(kCbOut);
    for (uint32_t i = 0; i < num_tiles; ++i) {
        cb_in.wait_front(1);
