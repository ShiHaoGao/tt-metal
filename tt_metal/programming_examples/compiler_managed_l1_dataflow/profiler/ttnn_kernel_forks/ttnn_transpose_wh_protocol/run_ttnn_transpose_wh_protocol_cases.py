...
from analyze_ttnn_transpose_wh_protocol import (
    parse_device_zones,
    parse_host_results,
...
def parse_shape(value):
    ...
    if len(parts) == 2:
        height_tiles, width_tiles = parts
        batches = 1
    elif len(parts) == 3:
        height_tiles, width_tiles, batches = parts
    else:
        raise argparse.ArgumentTypeError("shape must be HxW or HxWxB, for example 16x16 or 16x16x2")
    try:
        return int(height_tiles), int(width_tiles), int(batches)
    except ValueError as exc:
        raise argparse.ArgumentTypeError("shape dimensions must be integers") from exc


def parse_args():
    parser = argparse.ArgumentParser(description="Run TTNN transpose_wh protocol profiler cases.")
    parser.add_argument(
        "--exe",
        type=Path,
        default=Path("build_Release/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_transpose_wh_protocol"),
    )
    parser.add_argument("--out-dir", type=Path, default=Path("/tmp/ttnn_transpose_wh_protocol_cases"))
    parser.add_argument("--shapes", type=parse_shape, nargs="+", default=[(8, 8, 1), (16, 16, 1), (32, 32, 1)])
...
def run_case(args, height_tiles, width_tiles, batches, num_pages, mode):
    tiles = height_tiles * width_tiles * batches
    case_name = f"H{height_tiles}_W{width_tiles}_B{batches}_S{num_pages}_{mode}"
...
def build_critical_comparisons(critical_rows):
    ...
    for key, modes in sorted(grouped.items()):
        ...
        max_tiles_per_core = max(1, (key[3] + active_cores - 1) // active_cores)
            ...
            comparisons.append(
                {
                    ...
                    "delta_cycles_per_tile": delta / max(1, key[3]),
            ...
            comparisons.append(
                {
                    ...
                    "delta_us_per_tile": delta / max(1, key[3]),
...
def main():
    ...
    for height_tiles, width_tiles, batches in args.shapes:
        for num_pages in args.num_pages:
            for mode in args.modes:
                print(
                    f"running height_tiles={height_tiles} width_tiles={width_tiles} batches={batches} "
                    f"pages={num_pages} mode={mode}",
                    flush=True,
                )
                host_rows, host_summary, zone_rows, critical_rows = run_case(
                    args, height_tiles, width_tiles, batches, num_pages, mode
                )
