...
ZONE_RE = re.compile(
    r"^TTWH_(?P<mode>CB|STATIC_RUNTIME|STATIC_STREAMREG|STATIC_STREAMREG_CBREGS|STATIC_STREAMREG_CBREGS_COMPILETIME)_"
    r"(?P<stage>READER|WRITER|COMPUTE_UNPACK|COMPUTE_MATH|COMPUTE_PACK)$"
)
CASE_RE = re.compile(
    r"ttnn_transpose_wh_protocol: height_tiles=(?P<height_tiles>\d+), "
    r"width_tiles=(?P<width_tiles>\d+), batches=(?P<batches>\d+), "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize TTNN transpose_wh protocol profiler output.")
...
def summarize_host(rows):
    ...
    summaries = []
    for (height_tiles, width_tiles, batches, num_pages, core_grid_x, core_grid_y, mode), samples in sorted(
        grouped.items()
    ):
        tiles = samples[0]["tiles"]
