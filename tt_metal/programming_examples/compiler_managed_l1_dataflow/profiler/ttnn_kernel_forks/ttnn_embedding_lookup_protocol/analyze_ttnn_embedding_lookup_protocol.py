...
ZONE_RE = re.compile(r"^TEMB_(?P<mode>CB|STATIC_RUNTIME|STATIC_STREAMREG_CBREGS)_(?P<stage>READER|WRITER)$")
CASE_RE = re.compile(
    r"ttnn_embedding_lookup_protocol: rows=(?P<rows>\d+), vocab_size=(?P<vocab_size>\d+), "
    r"dim=(?P<dim>\d+), row_bytes=(?P<row_bytes>\d+), index_page_bytes=(?P<index_page_bytes>\d+), "
...
def parse_args():
    parser = argparse.ArgumentParser(description="Summarize TTNN embedding lookup protocol profiler output.")
...
def parse_host_results(path):
    ...
    with path.open() as log_file:
        for line in log_file:
            ...
            if case_match:
                metadata = {
                    "rows": int(case_match.group("rows")),
                    "vocab_size": int(case_match.group("vocab_size")),
                    "dim": int(case_match.group("dim")),
                    "row_bytes": int(case_match.group("row_bytes")),
                    "index_page_bytes": int(case_match.group("index_page_bytes")),
...
def summarize_host(rows):
    ...
    for row in rows:
        key = (row["rows"], row["vocab_size"], row["dim"], row["num_pages"], row["mode"])
        grouped[key].append(row)

    summaries = []
    for (rows_count, vocab_size, dim, num_pages, mode), samples in sorted(grouped.items()):
        ...
        summaries.append(
            {
                ...
                "dim": dim,
                "row_bytes": samples[0]["row_bytes"],
                "index_page_bytes": samples[0]["index_page_bytes"],
                "num_pages": num_pages,
                "core_grid_x": 1,
                "core_grid_y": 1,
                "mode": mode,
                ...
                "median_us_per_row": median(enqueue_values) / max(1, rows_count),
            }
        )
    return summaries
