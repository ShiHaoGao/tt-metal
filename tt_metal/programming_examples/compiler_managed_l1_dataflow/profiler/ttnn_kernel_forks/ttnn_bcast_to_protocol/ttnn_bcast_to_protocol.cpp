...
constexpr uint32_t kDefaultWidthTiles = 8;

constexpr std::string_view kReaderKernel =
    "tt_metal/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_kernel_forks/ttnn_bcast_to_protocol/kernels/dataflow/"
    "reader_interleaved_row_bcast_to_protocol.cpp";
constexpr std::string_view kWriterKernel =
    "tt_metal/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_kernel_forks/ttnn_bcast_to_protocol/kernels/dataflow/"
    "writer_interleaved_row_bcast_to_protocol.cpp";
constexpr std::string_view kComputeKernel =
    "tt_metal/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_kernel_forks/ttnn_bcast_to_protocol/kernels/compute/"
    "compute_interleaved_row_bcast_to_protocol.cpp";
...
struct Options {
    ...
    uint32_t width_tiles = kDefaultWidthTiles;
};
...
Options parse_options(int argc, char** argv) {
    ...
    if (options.tiles == 0 || options.width_tiles == 0 || options.num_pages == 0 || options.repeats == 0) {
        throw std::invalid_argument("--tiles, --width-tiles, --num-pages, and --repeats must be greater than zero");
    }
    ...
    if (options.tiles % options.width_tiles != 0) {
        throw std::invalid_argument("--tiles must be divisible by --width-tiles for the row-bcast shape");
    }
    return options;
}
...
uint32_t input_tiles(const Options& options) {
    return options.width_tiles;
}

uint32_t height_tiles(const Options& options) {
    return options.tiles / options.width_tiles;
}

uint32_t payload_size_bytes(uint32_t tiles) {
...
std::vector<bfloat16> make_input(uint32_t tiles) {
    ...
    for (uint32_t tile = 0; tile < tiles; ++tile) {
        const float value = 0.25f + static_cast<float>((tile * 17u) % 23u);
        for (uint32_t i = 0; i < kTileElements; ++i) {
            data[static_cast<size_t>(tile) * kTileElements + i] = bfloat16(value);
        }
    }
    return data;
}
...
void create_circular_buffers(
    ...
    CreateCircularBuffer(program, core, make_cb_config(CBIndex::c_1, dst_ring_buffer));
}
...
bool validate_result(
    ...
    float* max_abs_error) {
    if (input.size() != static_cast<size_t>(input_tiles(options)) * kTileElements ||
        result.size() != static_cast<size_t>(output_tiles(options)) * kTileElements) {
    ...
    for (uint32_t out_tile = 0; out_tile < output_tiles(options); ++out_tile) {
        const uint32_t input_tile = out_tile % options.width_tiles;
        const float expected = static_cast<float>(input[static_cast<size_t>(input_tile) * kTileElements]);
        for (uint32_t i = 0; i < kTileElements; ++i) {
            const size_t result_index = static_cast<size_t>(out_tile) * kTileElements + i;
            const float actual = static_cast<float>(result[result_index]);
            ...
            if (error > 3e-1f) {
                ...
                if (printed < 5) {
                    fmt::print(
                        stderr,
                        "Mismatch at output tile {} element {}: expected {}, got {}\n",
                        out_tile,
                        i,
                        expected,
                        actual);
...
std::array<uint32_t, 5> output_start_indices(const Options& options, uint32_t start_tile_id) {
    const uint32_t oHtWt = height_tiles(options) * options.width_tiles;
    const uint32_t tiles_per_batch = oHtWt;
    const uint32_t start_n = start_tile_id / tiles_per_batch;
    const uint32_t start_remaining = start_tile_id % tiles_per_batch;
    const uint32_t start_c = start_remaining / oHtWt;
    const uint32_t start_t = start_remaining % oHtWt;
    const uint32_t start_th = start_t / options.width_tiles;
    const uint32_t start_tw = start_t % options.width_tiles;
    return {start_n, start_c, start_t, start_th, start_tw};
}

std::vector<uint32_t> common_bcast_runtime_args(const Options& options, const CoreWork& work) {
    const auto [start_n, start_c, start_t, start_th, start_tw] = output_start_indices(options, work.start_tile);
    return {
        start_n,
        start_c,
        start_t,
        ...
        start_tw,
        work.tile_count,
        0,  // iHt * iWt * iC * (iN > 1), with iN=1 for this row-bcast shape.
        0,  // iHt * iWt * (iC > 1), with iC=1 for this row-bcast shape.
        1,
        1,
        height_tiles(options),
        options.width_tiles};
}

std::vector<uint32_t> reader_runtime_args(const Options& options, const CoreWork& work, uint32_t src_dram_addr) {
    auto args = common_bcast_runtime_args(options, work);
    args.insert(args.begin(), src_dram_addr);
    return args;
}

std::vector<uint32_t> writer_runtime_args(const Options& options, const CoreWork& work, uint32_t dst_dram_addr) {
    auto args = common_bcast_runtime_args(options, work);
    args.insert(args.begin(), dst_dram_addr);
    args.push_back(work.start_tile);
    return args;
}

std::vector<uint32_t> compute_runtime_args(const Options& options, const CoreWork& work) {
    return common_bcast_runtime_args(options, work);
}
...
RunResult run_one(
    ...
    auto src_dram_buffer = create_dram_buffer(mesh_device, payload_size_bytes(input_tiles(options)));
    auto dst_dram_buffer = create_dram_buffer(mesh_device, payload_size_bytes(output_tiles(options)));
    ...
    std::vector<uint32_t> reader_compile_time_args{static_cast<uint32_t>(CBIndex::c_0)};
    TensorAccessorArgs(src_dram_buffer).append_to(reader_compile_time_args);

    std::vector<uint32_t> writer_compile_time_args{static_cast<uint32_t>(CBIndex::c_1)};
    ...
    for (const auto& work : core_work) {
        std::vector<uint32_t> reader_args = reader_runtime_args(options, work, src_dram_addr);
        std::vector<uint32_t> writer_args = writer_runtime_args(options, work, dst_dram_addr);
        std::vector<uint32_t> compute_args = compute_runtime_args(options, work);
    ...
    float max_abs_error = 0.0f;
    const bool ok = validate_result(options, input, result, &max_abs_error);
    return {
        .mode = mode,
        .repeat = repeat,
        .enqueue_finish_us =
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        const auto input = make_input(input_tiles(options));

        fmt::print(
            "ttnn_bcast_to_protocol: tiles={}, input_tiles={}, width_tiles={}, height_tiles={}, tile_size={}, "
        ...
        for (uint32_t repeat = 0; repeat < options.repeats; ++repeat) {
            for (Mode mode : modes) {
                RunResult result = run_one(mesh_device, options, mode, repeat, input);
                pass &= result.ok;
                fmt::print(
