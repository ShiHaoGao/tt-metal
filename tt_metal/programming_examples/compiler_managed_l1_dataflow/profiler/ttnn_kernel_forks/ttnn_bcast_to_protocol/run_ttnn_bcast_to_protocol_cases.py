...
from analyze_ttnn_bcast_to_protocol import (
    parse_device_zones,
    parse_host_results,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run TTNN bcast_to row-bcast protocol profiler cases.")
    parser.add_argument(
        "--exe",
        type=Path,
        default=Path("build_Release/programming_examples/compiler_managed_l1_dataflow/profiler/ttnn_bcast_to_protocol"),
    )
    parser.add_argument("--out-dir", type=Path, default=Path("/tmp/ttnn_bcast_to_protocol_cases"))
    ...
    for tiles in args.tiles:
        for width_tiles in args.width_tiles:
            if tiles % width_tiles != 0:
                parser.error(f"--tiles value {tiles} must be divisible by --width-tiles value {width_tiles}")
    return args


def run_case(args, tiles, width_tiles, num_pages, mode):
    height_tiles = tiles // width_tiles
    case_name = f"T{tiles}_W{width_tiles}_H{height_tiles}_S{num_pages}_{mode}"
...
def build_critical_comparisons(critical_rows):
    ...
    for row in critical_rows:
        key = (row["tiles"], row["width_tiles"], row["height_tiles"], row["num_pages"], row["core_grid_x"], row["core_grid_y"])
...
def build_host_comparisons(host_summary_rows):
    ...
    for row in host_summary_rows:
        grouped[(row["tiles"], row["width_tiles"], row["height_tiles"], row["num_pages"], row["core_grid_x"], row["core_grid_y"])][row["mode"]] = row
        ...
        for width_tiles in args.width_tiles:
            for num_pages in args.num_pages:
                for mode in args.modes:
                    print(f"running tiles={tiles} width_tiles={width_tiles} pages={num_pages} mode={mode}", flush=True)
                    host_rows, host_summary, zone_rows, critical_rows = run_case(
                        args, tiles, width_tiles, num_pages, mode
                    )
