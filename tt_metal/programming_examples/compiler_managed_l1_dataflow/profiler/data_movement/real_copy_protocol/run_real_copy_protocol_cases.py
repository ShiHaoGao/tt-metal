...
from analyze_real_copy_protocol import parse_device_zones, parse_host_results, summarize_critical_stages, summarize_host, write_csv


def parse_args():
    parser = argparse.ArgumentParser(description="Run real_copy_protocol device-profiler cases one process at a time.")
    parser.add_argument(
        "--exe",
        type=Path,
        default=Path("build_Release/programming_examples/compiler_managed_l1_dataflow/profiler/real_copy_protocol"),
    )
    parser.add_argument("--out-dir", type=Path, default=Path("/tmp/real_copy_protocol_cases"))
    ...
    parser.add_argument(
        "--modes",
        nargs="+",
        default=["cb", "static-runtime", "static-compiletime", "static-streamreg-scratch"],
        choices=["cb", "static-runtime", "static-compiletime", "static-streamreg-scratch"],
    )
...
def build_critical_comparisons(critical_rows):
    ...
    for key, modes in sorted(grouped.items()):
        ...
        for static_mode in ["static-runtime", "static-compiletime", "static-streamreg-scratch"]:
