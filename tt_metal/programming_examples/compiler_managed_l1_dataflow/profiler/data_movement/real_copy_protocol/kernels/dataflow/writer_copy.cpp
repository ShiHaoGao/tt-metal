...
void kernel_main() {
    ...
    set_stream_sync(BENCH_STREAM_REG_READY_STREAM_ID, 0);
    set_stream_sync(BENCH_STREAM_REG_CONSUMED_STREAM_ID, 0);
    set_stream_sync(BENCH_STREAM_REG_START_STREAM_ID, BENCH_START_VALUE);
#else
    ...
    for (uint32_t i = 0; i < iterations; ++i) {
...
#if BENCH_USE_STREAM_REG_SYNC
        wait_min_stream(BENCH_STREAM_REG_READY_STREAM_ID, generation);
#else
        wait_min_local(ready_sem, generation);
#endif

        const uint32_t slot = i % num_pages;
        const uint32_t src_l1_addr = ring_addr + slot * page_size;
        ...
        noc_async_write_barrier();
#if BENCH_USE_STREAM_REG_SYNC
        set_stream_sync(BENCH_STREAM_REG_CONSUMED_STREAM_ID, generation);
#else
        set_local(consumed_sem, generation);
#endif
    }
#else
    ...
    DeviceZoneScopedN("RTCOPY_CB_WRITER");
    for (uint32_t i = 0; i < iterations; ++i) {
        const uint32_t global_tile_id = start_tile_id + i;
        cb_wait_front(kCbIn, kOneTile);
        const uint32_t src_l1_addr = get_read_ptr(kCbIn);
        noc_async_write(src_l1_addr, get_noc_addr(global_tile_id, dst_addrgen), page_size);
        noc_async_write_barrier();
        cb_pop_front(kCbIn, kOneTile);
    }
#endif
}
