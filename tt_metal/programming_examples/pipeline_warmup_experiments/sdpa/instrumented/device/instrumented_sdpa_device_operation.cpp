...
#include "tt_metal/programming_examples/pipeline_warmup_experiments/sdpa/instrumented/device/instrumented_sdpa_program_factory.hpp"
...
void InstrumentedSDPAOperation::validate_on_program_cache_miss(const InstrumentedSDPAParams& attrs, const InstrumentedSDPAInputs& tensors) {
...
InstrumentedSDPAOperation::spec_return_value_t InstrumentedSDPAOperation::compute_output_specs(
    const InstrumentedSDPAParams& attrs, const InstrumentedSDPAInputs& tensors) {
...
InstrumentedSDPAOperation::tensor_return_value_t InstrumentedSDPAOperation::create_output_tensors(
    const InstrumentedSDPAParams& attrs, const InstrumentedSDPAInputs& tensors) {
    sdpa_instrumentation::ScopedStageTimer timer("create_output_tensors");
    return create_device_tensor(compute_output_specs(attrs, tensors), tensors.q.device());
}

ttsl::hash::hash_t InstrumentedSDPAOperation::compute_program_hash(const InstrumentedSDPAParams& attrs, const InstrumentedSDPAInputs& tensors) {
    sdpa_instrumentation::ScopedStageTimer timer("compute_program_hash");
    ...
    operation::Hash hash = operation::hash_operation<InstrumentedSDPAOperation>(
        attrs.head_dim_v,
        attrs.scale,
...
tt::tt_metal::operation::OpPerformanceModelGeneral<InstrumentedSDPAOperation::tensor_return_value_t>
InstrumentedSDPAOperation::create_op_performance_model(
    const InstrumentedSDPAParams& args, const InstrumentedSDPAInputs& tensor_args, Tensor& output_tensor) {
    ...
    if (arch != tt::ARCH::WORMHOLE_B0 && arch != tt::ARCH::BLACKHOLE) {
        ...
        return operation::OpPerformanceModelGeneral<InstrumentedSDPAOperation::tensor_return_value_t>(
...
Tensor instrumented_sdpa(
    ...
    using OperationType = ttnn::prim::InstrumentedSDPAOperation;
