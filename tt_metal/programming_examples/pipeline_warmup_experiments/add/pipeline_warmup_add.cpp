...
constexpr std::string_view kReaderKernel =
    "tt_metal/programming_examples/pipeline_warmup_experiments/add/kernels/dataflow/reader_binary_tiles.cpp";
constexpr std::string_view kWriterKernel =
    "tt_metal/programming_examples/pipeline_warmup_experiments/add/kernels/dataflow/writer_tiles.cpp";
constexpr std::string_view kComputeKernel =
    "tt_metal/programming_examples/pipeline_warmup_experiments/add/kernels/compute/add_tiles.cpp";
...
struct Options {
    ...
    uint32_t prologue_tiles = 8;
    uint32_t prologue_block_tiles = 1;
    uint32_t steady_block_tiles = 4;
    uint32_t repeats = 1;
...
const char* mode_name(Mode mode) {
    switch (mode) {
        ...
        case Mode::CbDynamic: return "cb-dynamic";
...
std::optional<Mode> parse_mode(std::string_view mode) {
    ...
    if (mode == "cb-dynamic" || mode == "dynamic") {
        return Mode::CbDynamic;
    }
...
std::vector<Mode> modes_to_run(const std::string& mode) {
    if (mode == "all") {
        return {Mode::Cb, Mode::CbDynamic, Mode::StaticRuntime, Mode::StaticCompileTime, Mode::StaticStreamRegCbRegs};
    }
    auto parsed = parse_mode(mode);
    if (!parsed.has_value()) {
        throw std::invalid_argument(
            "Unknown --mode. Valid values are all, cb, cb-dynamic, static-runtime, static-compiletime, "
            "static-streamreg, static-streamreg-cbregs");
    }
    return {*parsed};
}
...
void print_usage(const char* argv0) {
    fmt::print(
        "Usage: {} [--mode=all|cb|cb-dynamic|static-runtime|static-compiletime|static-streamreg|static-streamreg-cbregs] [--tiles=N] "
        "[--num-pages=N] [--prologue-tiles=N] [--prologue-block-tiles=N] [--steady-block-tiles=N] "
...
Options parse_options(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (starts_with(arg, "--prologue-tiles=")) {
            options.prologue_tiles = parse_u32(arg.substr(std::string_view("--prologue-tiles=").size()), "prologue-tiles");
        } else if (starts_with(arg, "--prologue-block-tiles=")) {
            options.prologue_block_tiles =
                parse_u32(arg.substr(std::string_view("--prologue-block-tiles=").size()), "prologue-block-tiles");
        } else if (starts_with(arg, "--steady-block-tiles=")) {
            options.steady_block_tiles =
                parse_u32(arg.substr(std::string_view("--steady-block-tiles=").size()), "steady-block-tiles");
    ...
    if (options.prologue_block_tiles == 0 || options.steady_block_tiles == 0) {
        throw std::invalid_argument("--prologue-block-tiles and --steady-block-tiles must be greater than zero");
    }
...
uint32_t cb_pages(const Options& options, Mode mode) {
    if (mode == Mode::CbDynamic) {
        return std::max({options.num_pages, options.prologue_block_tiles, options.steady_block_tiles});
    }
    return options.num_pages;
}
...
std::map<std::string, std::string> kernel_defines(
    ...
    return {
        ...
        {"BENCH_DYNAMIC_BLOCKS", (mode == Mode::CbDynamic) ? "1" : "0"},
        ...
        {"BENCH_PROLOGUE_TILES", std::to_string(options.prologue_tiles)},
        {"BENCH_PROLOGUE_BLOCK_TILES", std::to_string(options.prologue_block_tiles)},
        {"BENCH_STEADY_BLOCK_TILES", std::to_string(options.steady_block_tiles)},
...
void create_circular_buffers(
    ...
    const uint32_t cb_size = cb_pages(options, mode) * kTileSizeBytes;
