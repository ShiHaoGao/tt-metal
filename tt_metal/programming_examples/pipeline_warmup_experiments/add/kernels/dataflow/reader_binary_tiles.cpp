...
#ifndef BENCH_PROLOGUE_TILES
#define BENCH_PROLOGUE_TILES 0
#endif

#ifndef BENCH_PROLOGUE_BLOCK_TILES
#define BENCH_PROLOGUE_BLOCK_TILES 1
#endif

#ifndef BENCH_STEADY_BLOCK_TILES
#define BENCH_STEADY_BLOCK_TILES 1
#endif
...
inline uint32_t dynamic_block_tiles(uint32_t block_start, uint32_t remaining) {
    uint32_t requested = block_start < BENCH_PROLOGUE_TILES ? BENCH_PROLOGUE_BLOCK_TILES : BENCH_STEADY_BLOCK_TILES;
...
void kernel_main() {
    ...
    DeviceZoneScopedN("RTADD_CB_DYNAMIC_READER");
#else
    DeviceZoneScopedN("RTADD_CB_READER");
#endif
    for (uint32_t block_start = 0; block_start < iterations;) {
        const uint32_t block_tiles =
            BENCH_DYNAMIC_BLOCKS ? dynamic_block_tiles(block_start, iterations - block_start) : kOneTile;

        cb_reserve_back(kCbIn0, block_tiles);
        uint32_t src0_l1_addr = get_write_ptr(kCbIn0);
        for (uint32_t j = 0; j < block_tiles; ++j) {
            const uint32_t global_tile_id = start_tile_id + block_start + j;
            noc_async_read(get_noc_addr(global_tile_id, src0_addrgen), src0_l1_addr + j * page_size, page_size);
        }
        noc_async_read_barrier();
        cb_push_back(kCbIn0, block_tiles);

        cb_reserve_back(kCbIn1, block_tiles);
        uint32_t src1_l1_addr = get_write_ptr(kCbIn1);
        for (uint32_t j = 0; j < block_tiles; ++j) {
            const uint32_t global_tile_id = start_tile_id + block_start + j;
            noc_async_read(get_noc_addr(global_tile_id, src1_addrgen), src1_l1_addr + j * page_size, page_size);
        }
        noc_async_read_barrier();
        cb_push_back(kCbIn1, block_tiles);

        block_start += block_tiles;
    }
#endif
}
