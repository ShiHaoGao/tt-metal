...
void kernel_main() {
    ...
    DeviceZoneScopedN("RTADD_CB_DYNAMIC_WRITER");
#else
    ...
    for (uint32_t block_start = 0; block_start < iterations;) {
        ...
        cb_wait_front(kCbOut, block_tiles);
        ...
        for (uint32_t j = 0; j < block_tiles; ++j) {
            ...
            noc_async_write(src_l1_addr + j * page_size, get_noc_addr(global_tile_id, dst_addrgen), page_size);
        }
        noc_async_write_barrier();
        cb_pop_front(kCbOut, block_tiles);
