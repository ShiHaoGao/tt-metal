...
void kernel_main() {
    ...
    DeviceZoneScopedN("RTADD_CB_DYNAMIC_COMPUTE_UNPACK");
#else
    DeviceZoneScopedN("RTADD_CB_COMPUTE_UNPACK");
#endif
#elif defined(TRISC_MATH)
#if BENCH_DYNAMIC_BLOCKS
    DeviceZoneScopedN("RTADD_CB_DYNAMIC_COMPUTE_MATH");
#else
    DeviceZoneScopedN("RTADD_CB_COMPUTE_MATH");
#endif
#elif defined(TRISC_PACK)
#if BENCH_DYNAMIC_BLOCKS
    DeviceZoneScopedN("RTADD_CB_DYNAMIC_COMPUTE_PACK");
#else
    ...
    for (uint32_t block_start = 0; block_start < iterations;) {
        ...
        cb_wait_front(kCbIn0, block_tiles);
        cb_wait_front(kCbIn1, block_tiles);
        cb_reserve_back(kCbOut, block_tiles);

        for (uint32_t j = 0; j < block_tiles; ++j) {
            tile_regs_acquire();
            add_tiles(kCbIn0, kCbIn1, j, j, kDstReg);
            tile_regs_commit();
        ...
        cb_push_back(kCbOut, block_tiles);
        cb_pop_front(kCbIn0, block_tiles);
        cb_pop_front(kCbIn1, block_tiles);
